 ...
 */
        struct vote_id_type {
            ...
            friend size_t hash_value(vote_id_type v) {
                return std::hash<uint32_t>()(v.content);
            }
            ...
            explicit vote_id_type(const std::string &serial) {
                ...
                if (colon != std::string::npos) {
                    *this = vote_id_type(vote_type(std::stoul(serial.substr(0, colon))), std::stoul(serial.substr(
                            colon + 1)));
                }
            }
...
namespace fc {
    ...
    void to_variant(const steemit::protocol::vote_id_type &var, fc::variant &vo);

    void from_variant(const fc::variant &var, steemit::protocol::vote_id_type &vo);

}

FC_REFLECT_TYPENAME(fc::flat_set<steemit::protocol::vote_id_type>)

FC_REFLECT_ENUM(steemit::protocol::vote_id_type::vote_type, (witness)(committee)(worker)(VOTE_TYPE_COUNT))
FC_REFLECT(steemit::protocol::vote_id_type, (content))