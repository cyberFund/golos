...
#define STEEMIT_DECLARE_PLUGIN_CREATOR(r, data, x) \
   std::shared_ptr< golos::application::abstract_plugin > BOOST_PP_CAT( create_, BOOST_PP_CAT( x,  _plugin ) )( golos::application::application* app );
...
namespace golos {
    namespace plugin {
        ...
        boost::container::flat_map<std::string, std::function<std::shared_ptr<golos::application::abstract_plugin>(golos::application::application *app)>> plugin_factories_by_name;

#define STEEMIT_REGISTER_PLUGIN_FACTORY(r, data, x) \
   plugin_factories_by_name[ #x ] = []( golos::application::application* app ) -> std::shared_ptr< golos::application::abstract_plugin >{ return BOOST_PP_CAT( create_, BOOST_PP_CAT( x, _plugin( app ) ) ); };
        ...
        std::shared_ptr<golos::application::abstract_plugin> create_plugin(const std::string &name, golos::application::application *app) {
            ...
            if (it == plugin_factories_by_name.end()) {
                return std::shared_ptr<golos::application::abstract_plugin>();
            }
