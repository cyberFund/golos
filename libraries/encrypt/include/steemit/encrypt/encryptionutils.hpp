...
#define STEEMIT_EL_GAMAL_GROUP_ELEMENT_SIZE 64 //bytes
#define STEEMIT_EL_GAMAL_CIPHERTEXT_SIZE (2 * STEEMIT_EL_GAMAL_GROUP_ELEMENT_SIZE) //bytes
#define STEEMIT_MESSAGE_SIZE 32 //bytes
#define STEEMIT_HASH_SIZE 32 //bytes
#define STEEMIT_PROOF_OF_DELIVERY_SIZE (STEEMIT_EL_GAMAL_GROUP_ELEMENT_SIZE * 5 + 2 * (STEEMIT_HASH_SIZE +1)) //bytes
const CryptoPP::Integer STEEMIT_EL_GAMAL_MODULUS_512(
...
const CryptoPP::Integer STEEMIT_EL_GAMAL_GROUP_GENERATOR(3);


const CryptoPP::Integer STEEMIT_SHAMIR_ORDER(
...
namespace steemit {
    namespace encrypt {
        ...
        enum encryption_results {
            ok, io_error, other_error, key_error,
        };
        ...
        public:
             ...
             */
            ShamirSecret(uint16_t _quorum, uint16_t _shares, DInteger _secret)
                    : secret(_secret), quorum(_quorum),
                      shares(_shares) {/*calculate_split();*/}
             ...
             */
            ShamirSecret(uint16_t _quorum, uint16_t _shares) : quorum(_quorum),
                                                               shares(_shares) {
            }
         ...
         * @return ok if successfull, or corresponding error code
         */
        encryption_results encrypt_with_proof(const point &message, const DInteger &privateKey, const DInteger &destinationPublicKey, const Ciphertext &incoming, Ciphertext &outgoing, DeliveryProof &proof);
         ...
         * @return ok if successfull, or corresponding error code
         */
        bool verify_delivery_proof(const DeliveryProof &proof, const Ciphertext &first, const Ciphertext &second, const DInteger &firstPulicKey, const DInteger &secondPublicKey);
