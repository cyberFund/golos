...
#define STEEMIT_CUSTODY_THREADS 4
...
namespace steemit {
    namespace encrypt {
        ...
        CustodyUtils::CustodyUtils() {
            pairing_init_set_str(pairing, _STEEMIT_PAIRING_PARAM_);

            element_init_G1(generator, pairing);

            element_set_str(generator, _STEEMIT_GENERATOR_, 10);
        }
        ...
        int CustodyUtils::get_n(std::fstream &file) {
            ...
            int n = length /
                    (STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD * STEEMIT_SECTORS);
            if (length %
                (STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD * STEEMIT_SECTORS)) {
                n += 1;
            }
        ...
        inline int CustodyUtils::get_data(std::fstream &file, uint32_t i, char buffer[]) {
            uint64_t position =
                    STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD * STEEMIT_SECTORS * i;

            file.seekg(0, file.end);
            uint64_t realLen = file.tellg();
            file.seekg(std::min(position, realLen), file.beg);

            if (realLen > ((long long) (file.tellg()) +
                           STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD *
                           STEEMIT_SECTORS)) {
                file.read(buffer, STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD *
                                  STEEMIT_SECTORS);
            } else {
                if (file.eof()) {
                    memset(buffer, 0, STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD *
                                      STEEMIT_SECTORS);
                } else {
                    int left = realLen - (long long) (file.tellg());
                    file.read(buffer, left);
                    memset(buffer + left, 0,
                           (STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD *
                            STEEMIT_SECTORS) - left);
                }
            }
            return 0;
        }
        ...
        inline int CustodyUtils::get_m(std::fstream &file, uint32_t i, uint32_t j, mpz_t &out) {
            mpz_init2(out, STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD * 8);
            uint64_t position = STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD *
                                (j + STEEMIT_SECTORS * i);
            char buffer[STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD];
            file.seekg(0, file.end);
            uint64_t realLen = file.tellg();
            if (realLen < position) { //we are either close or behinf EoF
                ...
                if (realLen < position +
                              STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD) { //we can't read the whole string
                    int left = realLen - (uint64_t) (file.tellg());
                    file.read(buffer, left);
                    memset(buffer + left, 0,
                           STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD - left);
                } else {
                    file.read(buffer, STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD);
                }
            }
            ...
            memcpy((char *) out->_mp_d, buffer,
                   STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD);
            out->_mp_size = STEEMIT_MP_SIZE_OF_NUMBER_IN_THE_FIELD;
            //mpz_import( out, STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD, 1, 1, 1, 0, buffer );
            return 1;
        }
        ...
        int CustodyUtils::get_sigma(uint64_t idx, mpz_t mi[], element_pp_t u_pp[], element_t pk, element_t out[]) {
            ...
            for (int j = 1; j < STEEMIT_SECTORS; j++) {
        ...
        int CustodyUtils::get_sigmas(std::fstream &file, const unsigned int n, element_t *u, element_t pk, element_t **sigmas) {
            element_t *ret = new element_t[n];
            //start threads
            fc::thread t[STEEMIT_CUSTODY_THREADS];
            fc::future<int> fut[STEEMIT_CUSTODY_THREADS];
            fc::future<void> fut_pp[STEEMIT_CUSTODY_THREADS];

            element_pp_t *u_pp = new element_pp_t[STEEMIT_SECTORS];
            for (int k = 0; k < STEEMIT_SECTORS; k++) {
                int idx = k % STEEMIT_CUSTODY_THREADS;
                fut_pp[idx] = t[idx].async([=]() {
                    element_pp_init(u_pp[k], u[k]);
                    return;
                });
            }
            //wait for the threads...
            for (int k = 0; k < STEEMIT_CUSTODY_THREADS; ++k) {
                fut_pp[k].wait();
            }

            //   std::cout<<"get_sigmas: n = "<<n<<" , cycles = " << cycles <<"\n";
            int total_thread_to_wait_for = std::min(n,
                                                    (const unsigned int) STEEMIT_CUSTODY_THREADS);

            for (uint64_t i = 0; i < n; i += STEEMIT_CUSTODY_THREADS) {
                int iterations = std::min((uint64_t) STEEMIT_CUSTODY_THREADS,
                                          n - i);
                for (int k = 0; k < iterations; ++k) {
                    ...
                    fut[k] = t[k].async([=]() {
                        mpz_t m[STEEMIT_SECTORS];
                        for (int i = 0; i < STEEMIT_SECTORS; ++i) {
                            mpz_init2(m[i],
                                      STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD * 8);
                            //mpz_import is too slow for our purposes - since we don't care about the exact parameters as much as about the uniqueness of the import, let's replace it with memcpy
                            memcpy((char *) m[i]->_mp_d, buffer + i *
                                                                  STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD,
                                   STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD);
                            m[i]->_mp_size = STEEMIT_MP_SIZE_OF_NUMBER_IN_THE_FIELD;
                            //mpz_import(m[i], STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD, 1, 1, 1, 0, buffer + i * STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD);
                        }
                        delete[] buffer;
        ...
        int CustodyUtils::compute_mu(std::fstream &file, unsigned int q, uint64_t indices[], element_t v[], element_t mu[]) {
            ...
            for (int j = 0; j < STEEMIT_SECTORS; j++) {
        ...
        int CustodyUtils::generate_query_from_seed(mpz_t seed, unsigned int q, unsigned int n, uint64_t indices[], element_t *v[]) {
            ...
            for (int i = 0; i < q; i++) {
                ...
                char *seed_str = new char[mpz_sizeinbase(seedForIteration, 16) +
                                          1];
                memset((char *) seed_str, 0,
                       mpz_sizeinbase(seedForIteration, 16) + 1);
                mpz_get_str(seed_str, 16, seedForIteration);
                fc::sha256 temp = fc::sha256::hash(seed_str, mpz_sizeinbase(
                        seedForIteration, 16));
                memcpy(digest, temp._hash, (4 * sizeof(uint64_t)));
        ...
        int CustodyUtils::verify(element_t sigma, unsigned int q, uint64_t *indices, element_t *v, element_t *u, element_t *mu, element_t pubk) {
            element_t res1;
        ...
        int CustodyUtils::unpack_proof(valtype proof, element_t &sigma, element_t **mu) {
            ...
            pointer += STEEMIT_SIZE_OF_POINT_ON_CURVE_COMPRESSED;

            element_t *ret = new element_t[STEEMIT_SECTORS];
            for (int i = 0; i < STEEMIT_SECTORS; i++) {
                element_init_Zr(ret[i], pairing);
                element_from_bytes(ret[i], proof.data() + pointer);
                pointer += STEEMIT_SIZE_OF_NUMBER_IN_THE_FIELD;
            }
            *mu = ret;
            return 0;
        }

        int CustodyUtils::compute_sigma(element_t sigmas[], unsigned int q, uint64_t indices[], element_t v[], element_t &sigma) {
            element_init_G1(sigma, pairing);
            element_set1(sigma);
        ...
        int CustodyUtils::verify_by_miner(const uint32_t &n, const char *u_seed, unsigned char *pubKey, unsigned char sigma[], std::vector<std::string> mus, mpz_t seed) {
            //prepate public_key and u
            element_t public_key;
            element_init_G1(public_key, pairing);
            element_from_bytes_compressed(public_key, pubKey);
            element_t u[STEEMIT_SECTORS];
            ...
            for (int i = 0; i < STEEMIT_SECTORS; i++) {
                ...
                unsigned char buffer[STEEMIT_SIZE_OF_MU];
                string_to_bytes(mus[i], buffer, STEEMIT_SIZE_OF_MU);
            ...
            clear_elements(u, STEEMIT_SECTORS);
            clear_elements(mu, STEEMIT_SECTORS);
            clear_elements(v, q);
        ...
        int CustodyUtils::create_custody_data(path content, uint32_t &n, char u_seed[], unsigned char pubKey[]) {
            //prepare the files
            std::fstream infile(content.c_str(),
                                std::fstream::binary | std::fstream::in);
            std::ofstream outfile(
                    (content.parent_path() / "content.cus").c_str(),
            ...
            char buffer[STEEMIT_SIZE_OF_POINT_ON_CURVE_COMPRESSED];
            for (int i = 0; i < n; i++) {
                element_to_bytes_compressed((unsigned char *) buffer,
                                            sigmas[i]);
                outfile.write(buffer,
                              STEEMIT_SIZE_OF_POINT_ON_CURVE_COMPRESSED);
            }
        ...
        int CustodyUtils::create_proof_of_custody(path content, const uint32_t n, const char u_seed[], unsigned char pubKey[], unsigned char sigma[], std::vector<std::string> &mus, mpz_t seed) {
            //open files
            std::fstream infile(content.c_str(),
                                std::fstream::binary | std::fstream::in);
            std::fstream cusfile(
            ...
            try {
                for (int i = 0; i < n; i++) {
                    ...
                    cusfile.read(&buffer[0],
                                 STEEMIT_SIZE_OF_POINT_ON_CURVE_COMPRESSED);
                    //      if(cusfile.readsome(buffer, STEEMIT_SIZE_OF_POINT_ON_CURVE_COMPRESSED) != STEEMIT_SIZE_OF_POINT_ON_CURVE_COMPRESSED )
                    //         return -9;
                    element_init_G1(sigmas[i], pairing);
                    element_from_bytes_compressed(sigmas[i],
            ...
            unsigned char buffer_mu[STEEMIT_SIZE_OF_MU];

            for (int i = 0; i < STEEMIT_SECTORS; i++) {
                memset(buffer_mu, 0, STEEMIT_SIZE_OF_MU);
                element_to_bytes(buffer_mu, mu[i]);
                std::string s = bytes_to_string(buffer_mu, STEEMIT_SIZE_OF_MU);
                mus.push_back(s);
            }
