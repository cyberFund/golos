...
#include <steemit/package/package.hpp>
...
using namespace steemit::package;


std::ostream &operator<<(std::ostream &os, const PackageInfo::DataState state) {
    switch (state) {
        case PackageInfo::DS_UNINITIALIZED:
            os << "UNINITIALIZED";
            break;
        case PackageInfo::INVALID:
            os << "INVALID";
            break;
        case PackageInfo::PARTIAL:
            os << "PARTIAL";
            break;
        case PackageInfo::UNCHECKED:
            os << "UNCHECKED";
            break;
        case PackageInfo::CHECKED:
            os << "CHECKED";
            break;
        default:
            os << "???";
...
std::ostream &operator<<(std::ostream &os, const PackageInfo::TransferState state) {
    switch (state) {
        case PackageInfo::TS_IDLE:
            os << "IDLE";
            break;
        case PackageInfo::DOWNLOADING:
            os << "DOWNLOADING";
            break;
        case PackageInfo::SEEDING:
            os << "SEEDING";
            break;
        default:
            os << "???";
...
std::ostream &operator<<(std::ostream &os, const PackageInfo::ManipulationState state) {
    switch (state) {
        case PackageInfo::MS_IDLE:
            os << "IDLE";
            break;
        case PackageInfo::PACKING:
            os << "PACKING";
            break;
        case PackageInfo::ENCRYPTING:
            os << "ENCRYPTING";
            break;
        case PackageInfo::STAGING:
            os << "STAGING";
            break;
        case PackageInfo::CHECKING:
            os << "CHECKING";
            break;
        case PackageInfo::DECRYPTING:
            os << "DECRYPTING";
            break;
        case PackageInfo::UNPACKING:
            os << "UNPACKING";
            break;
        case PackageInfo::DELETTING:
            os << "DELETTING";
            break;
        default:
            os << "???";
...
public:
    ...
    virtual void package_data_state_change(PackageInfo::DataState new_state) {
        std::clog << "Data state changed: " << new_state << std::endl;
    }

    virtual void package_transfer_state_change(PackageInfo::TransferState new_state) {
        std::clog << "Transfer state changed: " << new_state << std::endl;
    }

    virtual void package_manipulation_state_change(PackageInfo::ManipulationState new_state) {
        std::clog << "Manipulation state changed: " << new_state << std::endl;
    }

    virtual void package_creation_start() {
        std::clog << "Package creation started" << std::endl;
    }

    virtual void package_creation_progress() {
        std::clog << "Package creation progress" << std::endl;
    }

    virtual void package_creation_error(const std::string &error) {
        std::clog << "Package creation error: " << error << std::endl;
    }

    virtual void package_creation_complete() {
        std::clog << "Package creation complete" << std::endl;
    }

    virtual void package_restoration_start() {
        std::clog << "Package restoration started" << std::endl;
    }

    virtual void package_restoration_progress() {
        std::clog << "Package restoration progress" << std::endl;
    }

    virtual void package_restoration_error(const std::string &error) {
        std::clog << "Package restoration error: " << error << std::endl;
    }

    virtual void package_restoration_complete() {
        std::clog << "Package restoration complete" << std::endl;
    }

    virtual void package_extraction_start() {
        std::clog << "Package extraction started" << std::endl;
    }

    virtual void package_extraction_progress() {
        std::clog << "Package extraction progress" << std::endl;
    }

    virtual void package_extraction_error(const std::string &error) {
        std::clog << "Package extraction error: " << error << std::endl;
    }

    virtual void package_extraction_complete() {
        std::clog << "Package extraction complete" << std::endl;
    }

    virtual void package_check_start() {
        std::clog << "Package check started" << std::endl;
    }

    virtual void package_check_progress() {
        std::clog << "Package check progress" << std::endl;
    }

    virtual void package_check_error(const std::string &error) {
        std::clog << "Package check error: " << error << std::endl;
    }

    virtual void package_check_complete() {
        std::clog << "Package check complete" << std::endl;
    }
    ...
    virtual void package_seed_error(const std::string &error) {
        std::clog << "Package seed error: " << error << std::endl;
    }
...
void pm_sandbox() {
    ...
    const fc::sha256 key = fc::sha256::hash(
            std::string("some_string_to_use_as_a_key"));
    const bool block = false;


    {
        auto package_handle = package_manager.get_package(content_dir,
                                                          samples_dir, key);
...

        {
            ...
            std::cerr << "Package url:" << package_handle->get_url()
                      << std::endl;
            if (ex_ptr) {
                std::rethrow_exception(ex_ptr);
            }
        ...
        package_handle = package_manager.get_package(
