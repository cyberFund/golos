    ...
    namespace package {
        namespace detail {
            bool parse_ipfs_url(const std::string &url, std::string &obj_id) {
                ...
                if (url.substr(0, ipfs.size()) == ipfs) {
                    ...
                    boost::algorithm::trim_left_if(obj_id, [](char ch) {
                        return ch == '/';
                    });
                    boost::algorithm::trim_right_if(obj_id, [](char ch) {
                        return ch == '/';
                    });
                    return true;
                }
        ...
        IPFSDownloadPackageTask::IPFSDownloadPackageTask(PackageInfo &package)
                : detail::PackageTask(package),
                  _client(PackageManagerConfigurator::instance().get_ipfs_host(),
                          PackageManagerConfigurator::instance().get_ipfs_port()) {
        }
            ...
            for (auto nested_object : objects) {
                ...
                for (auto &link : links) {
                    ...
                    if ((int) link.at("Type") == 2) //file
                    {
                        ...
                        std::fstream myfile((dest_path / file_name).string(),
        ...
        void IPFSDownloadPackageTask::task() {
            ...
            const auto temp_dir_path = unique_path(
                    graphene::utilities::decent_path_finder::instance().get_decent_temp() /
                    "%%%%-%%%%-%%%%-%%%%");
            ...
            try {
                ...
                if (!detail::parse_ipfs_url(_package._url, obj_id)) {
                    FC_THROW("'${url}' is not an ipfs NURI",
                             ("url", _package._url));
                }
                ...
                paths_to_skip.clear();
                paths_to_skip.insert(
                        _package.get_package_state_dir(temp_dir_path));
                paths_to_skip.insert(
                        _package.get_lock_file_path(temp_dir_path));
                detail::move_all_except(temp_dir_path, package_dir,
                                        paths_to_skip);
                ...
                PACKAGE_INFO_CHANGE_TRANSFER_STATE(TS_IDLE);
                PACKAGE_INFO_GENERATE_EVENT(package_download_error,
                                            (ex.to_detail_string()));
                throw;
        ...
        void IPFSStartSeedingPackageTask::task() {
            ...
            try {
                ...
                detail::get_files_recursive_except(_package.get_package_dir(),
                                                   files, paths_to_skip);
...
#else
                    const auto file_rel_path = detail::get_relative(
                            package_base_path, file.lexically_normal());
                    files_to_add.push_back({file_rel_path.string(),
                                            ipfs::http::FileUpload::Type::kFileName,
                                            file.string()});
#endif
                }
                ...
                PACKAGE_INFO_CHANGE_TRANSFER_STATE(TS_IDLE);
                PACKAGE_INFO_GENERATE_EVENT(package_seed_error,
                                            (ex.to_detail_string()));
                throw;
            } catch (const std::exception &ex) {
                PACKAGE_INFO_CHANGE_TRANSFER_STATE(TS_IDLE);
