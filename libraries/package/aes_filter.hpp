    ...
    namespace iostreams {
        ...
        public:
            ...
            bool filter(const char *&src_begin, const char *src_end, char *&dest_begin, char *dest_end, bool flush) {
                ...
                std::cout << "ENCRYPTOR: SRC_SIZE=" << src_end - src_begin
                          << " DST_SIZE=" << dest_end - dest_begin << std::endl;
                ...
                const std::vector<char> &encrypted_data = aes_encrypt(_key,
                                                                      plain_data);

                std::cout << "ENCRYPTOR: ENC_DATA_SIZE="
                          << encrypted_data.size() << std::endl;
                ...
                *(int *) header.size = encrypted_data.size();
                ...
                int size_to_copy = std::min((int) encrypted_data.size(),
                                            (int) (dest_end - dest_begin -
                                                   sizeof(header)));

                std::strncpy(dest_begin + sizeof(header), encrypted_data.data(),
                             size_to_copy);
        ...
        public:
            ...
            bool filter(const char *&src_begin, const char *src_end, char *&dest_begin, char *dest_end, bool flush) {
                std::cout << "DECRYPTER: SRC_SIZE=" << src_end - src_begin
                          << " DST_SIZE=" << dest_end - dest_begin << std::endl;
                ...
                std::cout << "DECRYPTER: SIZE_TO_DECRYPT="
                          << *(int *) header.size << std::endl;

                const char *data_begin = src_begin + sizeof(header);
                const char *data_end =
                        src_begin + sizeof(header) + *(int *) header.size;

                std::vector<char> encrypted_data(data_begin, data_end);

                const std::vector<char> &plain_data = aes_decrypt(_key,
                                                                  encrypted_data);

                std::cout << "DECRYPTER: DEC_DATA_SIZE=" << plain_data.size()
                          << std::endl;

                int size_to_copy = std::min((int) plain_data.size(),
                                            (int) (dest_end - dest_begin -
                                                   sizeof(header)));
        ...
        template<typename Alloc = std::allocator<char> >
        struct basic_aes_encryptor
                : symmetric_filter<aes_encryptor_impl<Alloc>, Alloc> {
        private:
        ...
        public:
            ...
            basic_aes_encryptor(const fc::sha512 &key) : base_type(BUFFER_SIZE,
                                                                   key) {
            }
        };
        ...
        template<typename Alloc = std::allocator<char> >
        struct basic_aes_decrypter
                : symmetric_filter<aes_decrypter_impl<Alloc>, Alloc> {
        private:
        ...
        public:
            ...
            basic_aes_decrypter(const fc::sha512 &key) : base_type(BUFFER_SIZE,
                                                                   key) {
            }
        };
