    ...
    namespace package {
        namespace detail {
            ...
            bool is_nested(boost::filesystem::path nested, boost::filesystem::path base) {
                ...
                while (base_it != base.end() && nested_it != nested.end() &&
                       (*base_it) == (*nested_it)) {
                    ++base_it;
                    ++nested_it;
                }
            ...
            boost::filesystem::path get_relative(boost::filesystem::path from, boost::filesystem::path to) {
                ...
                while (from_it != from.end() && to_it != to.end() &&
                       (*from_it) == (*to_it)) {
                    ++from_it;
                    ++to_it;
                }
            ...
            void get_files_recursive(const boost::filesystem::path &dir, std::vector<boost::filesystem::path> &all_files) {
                ...
                if (!is_directory(dir)) {
                    FC_THROW("${path} does not point to an existing diectory",
                             ("path", dir.string()));
                }
            ...
            void get_files_recursive_except(const boost::filesystem::path &dir, std::vector<boost::filesystem::path> &some_files, const std::set<boost::filesystem::path> &paths_to_skip) {
                using namespace boost::filesystem;
                ...
                for (recursive_directory_iterator it(dir);
                     it != recursive_directory_iterator(); ++it) {
                    if (is_regular_file(*it) &&
                        paths_to_skip.find(*it) == paths_to_skip.end()) {
            ...
            void move_all_except(boost::filesystem::path from_dir, boost::filesystem::path to_dir, const std::set<boost::filesystem::path> &paths_to_skip) {
                ...
                for (recursive_directory_iterator it(from_dir);
                     it != recursive_directory_iterator(); ++it) {
                    if (is_directory(*it) && is_symlink(*it)) {
                        it.no_push();
                    }
                    ...
                    if (is_directory(*it) && !skip_this) {
                        create_directory(
                                to_dir / get_relative(from_dir, it->path()));
                        skip_this = true;
                    }

                    if (!skip_this) {
                        paths_to_rename[it->path()] =
                                to_dir / get_relative(from_dir, it->path());
                    }
                }
            ...
            void touch(const boost::filesystem::path &file_path) {
                ...
                if (!create_directories(file_dir) && !is_directory(file_dir)) {
                    FC_THROW("Unable to create directory ${path}",
                             ("path", file_dir.string()));
                }
            ...
            std::string get_proto(const std::string &url) {
                ...
                if (url.substr(0, ipfs.size()) == ipfs) {
                    return "ipfs";
                } else if (url.substr(0, magnet.size()) == magnet) {
                    return "magnet";
                } else {
                    return fc::url(url).proto();
                }
            }
            ...
            fc::ripemd160 calculate_hash(const boost::filesystem::path &file_path) {
                ...
                if (!fin.is_open()) {
                    FC_THROW("Unable to open file ${fn} for reading",
                             ("fn", file_path.string()));
                }
                ...
                const size_t source_size = boost::filesystem::file_size(
                        file_path);
                size_t total_read = 0;
                ...
                if (total_read != source_size) {
                    FC_THROW("Failed to read entire ${fn} file",
                             ("fn", file_path.string()));
                }
            ...
            PackageTask::PackageTask(PackageInfo &package) : _running(false),
                                                             _stop_requested(
                                                                     false),
                                                             _last_exception(
                                                                     nullptr),
