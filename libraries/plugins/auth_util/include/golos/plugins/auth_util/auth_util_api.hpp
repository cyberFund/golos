...
namespace golos {
    namespace plugin {
        namespace auth_util {
            ...
            class auth_util_api {
            public:
                auth_util_api(const golos::application::api_context &ctx);
...
FC_REFLECT((golos::plugin::auth_util::check_authority_signature_params),
        (account_name)
                (level)
                (dig)
                (sigs)
)
FC_REFLECT((golos::plugin::auth_util::check_authority_signature_result),
        (keys)
)

FC_API(golos::plugin::auth_util::auth_util_api,
