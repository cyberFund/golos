    ...
    namespace witness_plugin {
        ...
        void witness_plugin::plugin_set_program_options(
            ...
            command_line_options.add_options()
                    ...
                    ("witness,w", boost::program_options::value<std::vector<std::string>>()->composing()->multitoken(),
                            ("name of witness controlled by this node (e.g. " +
                             witness_id_example + " )").c_str())
                    ("miner,m", boost::program_options::value<std::vector<std::string>>()->composing()->multitoken(), "name of miner and its private key (e.g. [\"account\",\"WIF PRIVATE KEY\"] )")
                    ("mining-threads,t", boost::program_options::value<uint32_t>(), "Number of threads to use for proof of work mining")
                    ("private-key", boost::program_options::value<std::vector<std::string>>()->composing()->multitoken(), "WIF PRIVATE KEY to be used by one or more witnesses or miners")
