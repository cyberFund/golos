        ...
        namespace debug_node {
            ...
            namespace detail {
                ...
                void debug_private_key_storage::maybe_get_private_key(fc::optional<fc::ecc::private_key> &result,
                    ...
                    fc::ecc::private_key gen_priv = fc::ecc::private_key::regenerate(
                            fc::sha256::hash(dev_key_prefix + account_name));
                    chain::public_key_type gen_pub = gen_priv.get_public_key();
                    key_table[gen_pub] = gen_priv;
                    if ((pubkey == chain::public_key_type()) || (gen_pub == pubkey)) {
                        result = gen_priv;
                        return;
                    }
                ...
                void debug_node_api_impl::debug_get_dev_key(get_dev_key_result &result, const get_dev_key_args &args) {
                    fc::ecc::private_key priv = fc::ecc::private_key::regenerate(
                            fc::sha256::hash(key_storage.dev_key_prefix + args.name));
                ...
                void debug_node_api_impl::debug_mine(debug_mine_result &result, const debug_mine_args &args) {
                    ...
                    if (db->has_hardfork(STEEMIT_HARDFORK_0_17)) {
                        ...
                        if (args.props.valid()) {
                            op.props = {protocol::asset<0, 16, 0>(args.props->account_creation_fee.amount,
                                                                  args.props->account_creation_fee.symbol_name()),
                                        args.props->maximum_block_size, args.props->sbd_interest_rate};
                        } else {
                            op.props = {protocol::asset<0, 16, 0>(
                                    db->get_witness_schedule_object().median_props.account_creation_fee.amount,
                                    db->get_witness_schedule_object().median_props.account_creation_fee.symbol_name()),
                                        db->get_witness_schedule_object().median_props.maximum_block_size,
                                        db->get_witness_schedule_object().median_props.sbd_interest_rate};
                        }
                ...
                uint32_t debug_node_api_impl::debug_push_blocks(const std::string &src_filename, uint32_t count,
                    ...
                    if (fc::exists(src_path) && fc::exists(index_path) && !fc::is_directory(src_path) &&
                ...
                uint32_t debug_node_api_impl::debug_generate_blocks(const std::string &debug_key, uint32_t count) {
                    return get_plugin()->debug_generate_blocks(debug_key, count, chain::database::skip_nothing, 0,
                                                               &key_storage);
                }
                ...
                uint32_t debug_node_api_impl::debug_generate_blocks_until(const std::string &debug_key,
                    ...
                    return get_plugin()->debug_generate_blocks_until(debug_key, head_block_time, generate_sparsely,
                                                                     chain::database::skip_nothing, &key_storage);
                }
