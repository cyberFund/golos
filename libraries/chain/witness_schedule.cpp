        ...
        void update_median_witness_props(database &db) {
            ...
            db.modify(wso, [&](witness_schedule_object &_wso) {
                ...
                _wso.median_props.asset_creation_fee = {
                        std::accumulate(active.begin(), active.end(), asset<0, 17, 0>::amount_container_type(),
                                        [&](const asset<0, 17, 0>::amount_container_type &current,
                                            const witness_object *b) {
                                            return current + b->props.asset_creation_fee.amount;
        ...
        void update_witness_schedule4(database &db) {
            ...
            for (auto &processed_witnesse : processed_witnesses) {
                auto new_virtual_scheduled_time =
                        new_virtual_time + VIRTUAL_SCHEDULE_LAP_LENGTH2 / (processed_witnesse->votes.value + 1);
