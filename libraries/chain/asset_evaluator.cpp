    ...
    namespace chain {
        ...
        void asset_create_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &op) {
            try {
                ...
                auto &asset_indx = this->db.get_index<asset_index>().indices().get<by_asset_name>();
            ...
            try {
                this->db.create<asset_dynamic_data_object>([&](asset_dynamic_data_object &a) {
                    a.asset_name = op.asset_name;
                    a.current_supply = 0;
                    a.fee_pool = 0; //op.calculate_fee(this->db.current_fee_schedule()).value / 2;
                });

                if (op.bitasset_opts.valid()) {
                    this->db.create<asset_bitasset_data_object>([&](asset_bitasset_data_object &a) {
                        a.asset_name = op.asset_name;
                ...
                this->db.create<asset_object>([&](asset_object &a) {
                    a.issuer = op.issuer;
                    a.asset_name = op.asset_name;
        ...
        void asset_fund_fee_pool_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            try {
                ...
                this->db.adjust_balance(this->db.get_account(o.from_account), -protocol::asset(o.amount, o.asset_name));
        ...
        void asset_update_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            ...
            try {
                ...
                if (o.new_options.flags & disable_force_settle && asset_to_update->can_force_settle()) {
                    const auto &idx = this->db.get_index<force_settlement_index>().indices().get<by_expiration>();
                ...
                this->db.modify(*asset_to_update, [&](asset_object &a) {
                    if (o.new_issuer) {
        ...
        void asset_update_feed_producers_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            ...
            try {
                this->db.modify(*bitasset_to_update, [&](asset_bitasset_data_object &a) {
                    //This is tricky because I have a set of publishers coming in, but a map of publisher to feed is storethis->db.
        ...
        void asset_global_settle_evaluator<Major, Hardfork, Release>::do_apply(operation_type &op) {
            try {
                ...
                const auto &idx = this->db.get_index<call_order_index>().indices().get<by_collateral>();
                assert(!idx.empty());
                auto itr = idx.lower_bound(boost::make_tuple(
                        price::min(this->db.get_asset_bitasset_data(asset_to_settle->asset_name).options.short_backing_asset,
                                   op.asset_to_settle)));
        ...
        void asset_force_settle_evaluator<Major, Hardfork, Release>::do_apply(operation_type &op) {
            ...
            try {
                ...
                if (bitasset.has_settlement()) {
                    ...
                    this->db.create<force_settlement_object>([&](force_settlement_object &s) {
                        s.owner = op.account;
        ...
        void asset_publish_feeds_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            try {
                ...
                if (base.options.flags & witness_fed_asset) {
                    const account_authority_object &witness_authority = this->db.get<account_authority_object, by_account>(
                    ...
                    const account_authority_object &committee_authority = this->db.get<account_authority_object, by_account>(
            ...
            try {
                ...
                if (!(old_feed == bad.current_feed)) {
                    if (bad.has_settlement()) {
                        const auto &mia_dyn = this->db.get_asset_dynamic_data(base.asset_name);
