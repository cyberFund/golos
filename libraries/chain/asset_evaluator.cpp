    ...
    namespace chain {
            ...
            try {
                ...
                if (op.bitasset_opts) {
                    ...
                    if (backing.is_market_issued()) {
                        const asset_bitasset_data_object &backing_bitasset_data = db.get_asset_bitasset_data(
                                backing.asset_name);
                        const asset_object &backing_backing = db.get_asset(
                        ...
                        FC_ASSERT(db.get_account(op.issuer).name != STEEMIT_COMMITTEE_ACCOUNT ||
                ...
                if (op.is_prediction_market) {
                    ...
                    FC_ASSERT(op.precision == db.get_asset(op.bitasset_opts->short_backing_asset).precision);
                }
            ...
            try {
                ...
                if (o.new_issuer) {
                    ...
                    if (a.is_market_issued() && *o.new_issuer == STEEMIT_COMMITTEE_ACCOUNT) {
                        const asset_object &backing = db.get_asset(
                                db.get_asset_bitasset_data(a.asset_name).options.short_backing_asset);
                        if (backing.is_market_issued()) {
                            const asset_object &backing_backing = db.get_asset(
                                    db.get_asset_bitasset_data(backing.asset_name).options.short_backing_asset);
        ...
        void asset_update_bitasset_evaluator::do_apply(const asset_update_bitasset_operation &o) {
            try {
                ...
                if (o.new_options.short_backing_asset != b.options.short_backing_asset) {
                    FC_ASSERT(db.get_asset_dynamic_data(a.asset_name).current_supply == 0);
        ...
        void asset_global_settle_evaluator::do_apply(const asset_global_settle_evaluator::operation_type &op) {
            try {
                ...
                FC_ASSERT(db.get_asset_dynamic_data(asset_to_settle->asset_name).current_supply > 0);
                const auto &idx = db.get_index<call_order_index>().indices().get<by_collateral>();
                assert(!idx.empty());
                auto itr = idx.lower_bound(boost::make_tuple(
                        price::min(db.get_asset_bitasset_data(asset_to_settle->asset_name).options.short_backing_asset,
                                   op.asset_to_settle)));
        ...
        void asset_settle_evaluator::do_apply(const asset_settle_evaluator::operation_type &op) {
            try {
                ...
                FC_ASSERT(db.get_balance(db.get_account(op.account), *asset_to_settle) >= op.amount);
            } FC_CAPTURE_AND_RETHROW((op))
            ...
            try {
                ...
                if (bitasset.has_settlement()) {
                    ...
                    db.create<force_settlement_object>([&](force_settlement_object &s) {
                        ...
                        s.balance = op.amount;
                        s.settlement_date = db.head_block_time() + db.get_asset_bitasset_data(
        ...
        void asset_publish_feeds_evaluator::do_apply(const asset_publish_feed_operation &o) {
            try {
                ...
                if (base.options.flags & witness_fed_asset) {
                    const account_authority_object &witness_authority = db.get<account_authority_object, by_account>(
                    ...
                    const account_authority_object &committee_authority = db.get<account_authority_object, by_account>(
            ...
            try {
                ...
                if (!(old_feed == bad.current_feed)) {
                    if (bad.has_settlement()) {
                        const auto &mia_dyn = db.get_asset_dynamic_data(base.asset_name);
