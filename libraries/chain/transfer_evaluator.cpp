...
namespace steemit {
    namespace chain {
        ...
        void transfer_evaluator<Major, Hardfork, Release>::do_apply(const protocol::transfer_operation<Major, Hardfork, Release> &o) {
            ...
            const auto &to_account = this->db.template get_account(o.to);

            const asset_object &asset_type = this->db.template get_asset(o.amount.symbol);

            STEEMIT_ASSERT(this->db.template is_authorized_asset(from_account, asset_type), transfer_from_account_not_whitelisted,
                           "'from' account ${from} is not whitelisted for asset ${asset}",
                           ("from", o.from)("asset", o.amount.symbol));
            STEEMIT_ASSERT(this->db.template is_authorized_asset(to_account, asset_type), transfer_to_account_not_whitelisted,
            ...
            if (from_account.active_challenged) {
                this->this->db.template modify(from_account, [&](account_object &a) {
                    a.active_challenged = false;
                    a.last_active_proved = this->this->db.template head_block_time();
                });
            }

            FC_ASSERT(this->this->db.template get_balance(from_account, o.amount.symbol) >= o.amount,
                      "Account does not have sufficient funds for transfer.");

            this->this->db.template adjust_balance(from_account, -o.amount);
            this->this->db.template adjust_balance(to_account, o.amount);
        }
        ...
        void transfer_to_vesting_evaluator<Major, Hardfork, Release>::do_apply(const protocol::transfer_to_vesting_operation<Major, Hardfork, Release> &o) {
            const auto &from_account = this->this->db.template get_account(o.from);
            const auto &to_account = o.to.size() ? this->this->db.template get_account(o.to) : from_account;

            FC_ASSERT(this->this->db.template get_balance(from_account, STEEM_SYMBOL_NAME) >= o.amount,
                      "Account does not have sufficient {a} for transfer.", ("a", STEEM_SYMBOL_NAME));
            this->this->db.template adjust_balance(from_account, -o.amount);
            this->this->db.template create_vesting(to_account, o.amount);
        }
        ...
        void transfer_to_savings_evaluator<Major, Hardfork, Release>::do_apply(
            ...
            const auto &from = this->this->db.template get_account(op.from);
            const auto &to = this->this->db.template get_account(op.to);
            FC_ASSERT(this->this->db.template get_balance(from, op.amount.symbol) >= op.amount,
                      "Account does not have sufficient funds to transfer to savings.");

            this->this->db.template adjust_balance(from, -op.amount);
            this->this->db.template adjust_savings_balance(to, op.amount);
        }
        ...
        void transfer_from_savings_evaluator<Major, Hardfork, Release>::do_apply(
            ...
            const auto &from = this->this->db.template get_account(op.from);
            this->this->db.template get_account(op.to); // Verify to account exists
            ...
            FC_ASSERT(this->this->db.template get_savings_balance(from, op.amount.symbol) >= op.amount);
            this->this->db.template adjust_savings_balance(from, -op.amount);
            this->this->db.template create<savings_withdraw_object>([&](savings_withdraw_object &s) {
                s.from = op.from;
                s.to = op.to;
                s.amount = op.amount;
                ...
                s.complete = this->this->db.template head_block_time() + STEEMIT_SAVINGS_WITHDRAW_TIME;
            });

            this->this->db.template modify(from, [&](account_object &a) {
        ...
        void cancel_transfer_from_savings_evaluator<Major, Hardfork, Release>::do_apply(
            ...
            const auto &swo = this->this->db.template get_savings_withdraw(op.from, op.request_id);
            this->this->db.template adjust_savings_balance(this->this->db.template get_account(swo.from), swo.amount);
            this->this->db.template remove(swo);
        ...
        void override_transfer_evaluator<Major, Hardfork, Release>::do_apply(
            ...
            try {
                ...
                FC_ASSERT(this->db.template get_balance(from_account, asset_type).amount >= o.amount.amount, "",
                          ("total_transfer", o.amount)("balance", this->db.template get_balance(from_account, asset_type).amount));

                this->db.template adjust_balance(this->db.template get_account(o.from), -o.amount);
                this->db.template adjust_balance(this->db.template get_account(o.to), o.amount);
