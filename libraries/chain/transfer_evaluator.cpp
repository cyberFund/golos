...
namespace steemit {
    namespace chain {
        ...
        void transfer_evaluator<Major, Hardfork, Release>::do_apply(
                const protocol::transfer_operation<Major, Hardfork, Release> &o) {
            ...
            STEEMIT_ASSERT(this->db.template is_authorized_asset(from_account, asset_type),
            ...
            STEEMIT_ASSERT(this->db.template is_authorized_asset(to_account, asset_type),
            ...
            if (asset_type.is_transfer_restricted()) {
                STEEMIT_ASSERT(from_account.name == asset_type.issuer || to_account.name == asset_type.issuer,
                               typename BOOST_IDENTITY_TYPE((exceptions::operations::transfer::restricted_transfer_asset<Major, Hardfork, Release>)), "Asset {asset} has transfer_restricted flag enabled",
            ...
            this->db.template adjust_balance(from_account, -o.amount);
            this->db.template adjust_balance(to_account, o.amount);
        }

        template<uint8_t Major, uint8_t Hardfork, uint16_t Release>
        void transfer_to_vesting_evaluator<Major, Hardfork, Release>::do_apply(
                const protocol::transfer_to_vesting_operation<Major, Hardfork, Release> &o) {
            const auto &from_account = this->db.template get_account(o.from);
            const auto &to_account = o.to.size() ? this->db.template get_account(o.to) : from_account;

            FC_ASSERT(this->db.template get_balance(from_account, STEEM_SYMBOL_NAME) >= o.amount,
                      "Account does not have sufficient {a} for transfer.", ("a", STEEM_SYMBOL_NAME));
            this->db.template adjust_balance(from_account, -o.amount);
            this->db.template create_vesting(to_account, o.amount);
        }
        ...
        void transfer_to_savings_evaluator<Major, Hardfork, Release>::do_apply(
            ...
            const auto &from = this->db.template get_account(op.from);
            const auto &to = this->db.template get_account(op.to);
            FC_ASSERT(this->db.template get_balance(from, op.amount.symbol) >= op.amount,
                      "Account does not have sufficient funds to transfer to savings.");

            this->db.template adjust_balance(from, -op.amount);
            this->db.template adjust_savings_balance(to, op.amount);
        }
        ...
        void transfer_from_savings_evaluator<Major, Hardfork, Release>::do_apply(
            ...
            const auto &from = this->db.template get_account(op.from);
            this->db.template get_account(op.to); // Verify to account exists
            ...
            FC_ASSERT(this->db.template get_savings_balance(from, op.amount.symbol) >= op.amount);
            this->db.template adjust_savings_balance(from, -op.amount);
        ...
        void cancel_transfer_from_savings_evaluator<Major, Hardfork, Release>::do_apply(
            ...
            const auto &swo = this->db.template get_savings_withdraw(op.from, op.request_id);
        ...
        void override_transfer_evaluator<Major, Hardfork, Release>::do_apply(
            ...
            try {
                ...
                STEEMIT_ASSERT(asset_type.can_override(),
                               typename BOOST_IDENTITY_TYPE((exceptions::operations::override_transfer::not_permitted<Major, Hardfork, Release>)),
