...
namespace steemit {
    namespace chain {
        ...
        optional<asset> PriceRegions::GetPrice(uint32_t region_code) const {
            ...
            auto it_single_price = map_price.find(
                    uint32_t(RegionCodes::OO_none));
            ...
            auto it_default_price = map_price.find(
                    uint32_t(RegionCodes::OO_all));
        ...
        void PriceRegions::SetSimplePrice(asset const &price) {
            map_price.clear();
            map_price.insert(
                    std::make_pair(uint32_t(RegionCodes::OO_none), price));
        }
