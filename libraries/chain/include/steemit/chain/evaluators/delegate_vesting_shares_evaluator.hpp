...
namespace steemit {
    namespace chain {
        ...
        class delegate_vesting_shares_evaluator : public evaluator_impl<database_set,
                delegate_vesting_shares_evaluator> {
        public:
            typedef protocol::delegate_vesting_shares_operation operation_type;

            template<typename Database> delegate_vesting_shares_evaluator(Database &db) : evaluator_impl<database_set,
