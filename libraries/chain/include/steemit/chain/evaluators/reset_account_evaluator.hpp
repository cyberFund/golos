...
namespace steemit {
    namespace chain {
        ...
        class reset_account_evaluator : public evaluator_impl<database_set, reset_account_evaluator> {
        public:
            typedef protocol::reset_account_operation operation_type;

            template<typename Database>
            reset_account_evaluator(Database &db) : evaluator_impl<database_set, reset_account_evaluator>(db) {
            }
