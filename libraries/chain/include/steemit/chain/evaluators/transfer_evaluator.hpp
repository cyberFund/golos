...
namespace steemit {
    namespace chain {
        ...
        class transfer_evaluator : public evaluator_impl<database_t, transfer_evaluator> {
        public:
            typedef protocol::transfer_operation operation_type;

            template<typename Database>
            transfer_evaluator(Database &db) : evaluator_impl<database_t, transfer_evaluator>(db) {
            }
