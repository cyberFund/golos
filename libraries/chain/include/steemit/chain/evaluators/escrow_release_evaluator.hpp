...
namespace steemit {
    namespace chain {
        ...
        class escrow_release_evaluator : public evaluator_impl<database_set, escrow_release_evaluator> {
        public:
            typedef protocol::escrow_release_operation operation_type;

            template<typename Database>
            escrow_release_evaluator(Database &db) : evaluator_impl<database_set, escrow_release_evaluator>(db) {
            }
