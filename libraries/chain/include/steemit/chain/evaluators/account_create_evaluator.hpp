...
namespace steemit {
    namespace chain {
        ...
        class account_create_evaluator : public evaluator_impl<database_set, account_create_evaluator> {
        public:
            typedef protocol::account_create_operation operation_type;

            template<typename Database>
            account_create_evaluator(Database &db) : evaluator_impl<database_set, account_create_evaluator>(db) {
            }
