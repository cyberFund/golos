...
namespace steemit {
    namespace chain {
        ...
        class convert_evaluator : public evaluator_impl<database_t, convert_evaluator> {
        public:
            typedef protocol::convert_operation operation_type;

            template<typename Database>
            convert_evaluator(Database &db) : evaluator_impl<database_t, convert_evaluator>(db) {
            }
