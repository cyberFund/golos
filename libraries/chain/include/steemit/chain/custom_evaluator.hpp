...
namespace steemit {
    namespace chain {
        ...
        public:
            ...
            custom_evaluator(database &db) : evaluator<custom_evaluator>(db) {
            }

            void do_apply(const protocol::custom_operation &o);
        };

        class custom_json_evaluator : public evaluator<custom_json_evaluator> {
        public:
            typedef protocol::custom_json_operation operation_type;

            custom_json_evaluator(database &db) : evaluator<custom_json_evaluator>(db) {
            }

            void do_apply(const protocol::custom_json_operation &o);
        };

        class custom_binary_evaluator : public evaluator<custom_binary_evaluator> {
        public:
            typedef protocol::custom_binary_operation operation_type;

            custom_binary_evaluator(database &db) : evaluator<custom_binary_evaluator>(db) {
            }
