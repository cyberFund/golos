...
namespace steemit {
    namespace chain {
        using namespace steemit::encrypt;
        ...
        public:
            ...
            void load(string const &str_synopsis) {
                ...
                if (variant_synopsis.is_object()) {
                    ...
                    if (variant_synopsis.get_object().contains(
                            str_name.c_str())) {
                ...
                if (variant_value.is_array()) {
                    ...
                    value_type item;
                    ContentObjectPropertyBase::convert_from_string(str_value,
                                                                   item);
                    m_arrValues.push_back(item);
                }

                if (bFallBack && m_arrValues.empty()) {
                    value_type item;
                    ContentObjectPropertyBase::convert_from_string(str_synopsis,
                                                                   item);
                    m_arrValues.push_back(item);
                }

                if (is_default(0) && m_arrValues.empty()) {
                    string str_value;
                    value_type item;
                    ContentObjectPropertyBase::convert_from_string(str_value,
                                                                   item);
                ...
                if (is_unique(0) && 1 < m_arrValues.size()) {
            ...
            void save(string &str_synopsis) const {
                ...
                if (is_unique(0) && 1 < arrValues.size()) {
                    arrValues.resize(1);
                }

                if (is_default(0) && arrValues.empty()) {
                    string str_value;
                    value_type item;
                    ContentObjectPropertyBase::convert_from_string(str_value,
                                                                   item);
                ...
                if (1 == arrValues.size()) {
                    ...
                    string str_value;
                    ContentObjectPropertyBase::convert_to_string(item,
                                                                 str_value);
        ...
        public:
            ContentObjectTypeValue(EContentObjectApplication appID = EContentObjectApplication::DecentCore, EContentObjectType typeID = EContentObjectType::None) {
                init(appID, typeID);
            }
        ...
        private:
            void init(EContentObjectApplication appID = EContentObjectApplication::DecentCore, EContentObjectType typeID = EContentObjectType::None) {
                if (type.size() == 0) {
                    type.push_back(static_cast<uint32_t>(appID));
                }
                if (type.size() == 1 && typeID != EContentObjectType::None) {
        ...
        class ContentObjectType
                : public ContentObjectPropertyBase<ContentObjectTypeValue, ContentObjectType> {
        public:
        ...
        class ContentObjectTitle
                : public ContentObjectPropertyBase<string, ContentObjectTitle> {
        public:
        ...
        class ContentObjectDescription
                : public ContentObjectPropertyBase<string, ContentObjectDescription> {
        public:
            using meta_default = bool;
            using meta_unique = bool;
        ...
        class ContentObjectISBN
                : public ContentObjectPropertyBase<string, ContentObjectISBN> {
        public:
            using meta_unique = bool;
            ...
            typename P::value_type get() const {
                ...
                if (1 != property.m_arrValues.size()) {
                    throw std::runtime_error(
                            "ContentObjectPropertyManager expects ideal conditions for now");
                }
        ...
        class content_object
                : public steemit::db::abstract_object<content_object> {
        public:
        ...
        typedef multi_index_container <content_object, indexed_by<
                ordered_unique < tag <
                by_id>, member<object, object_id_type, &object::id>>,


        ordered_non_unique <tag<by_author>, member<content_object, account_id_type, &content_object::author>>,
        ordered_unique <tag<by_URI>, member<content_object, string, &content_object::URI>>,
        ordered_non_unique <tag<by_price>, const_mem_fun<content_object, share_type, &content_object::get_price_amount_template<RegionCodes::OO_none>>>,
        ordered_non_unique <tag<by_size>, member<content_object, uint64_t, &content_object::size>>,
        ordered_non_unique <tag<by_AVG_rating>, member<content_object, uint64_t, &content_object::AVG_rating>>,
        ordered_non_unique <tag<by_times_bought>, member<content_object, uint32_t, &content_object::times_bought>, std::greater<uint32_t>>,

        ordered_non_unique <tag<by_expiration>, member<content_object, time_point_sec, &content_object::expiration>>,

        ordered_non_unique <tag<by_created>, member<content_object, time_point_sec, &content_object::created>>
...
FC_REFLECT(steemit::chain::content_object, (author)(co_authors)(expiration)(created)(price)(size)(
                           synopsis)(URI)(quorum)(key_parts)(_hash)(last_proof)(
                           is_blocked)(AVG_rating)(num_of_ratings)(
                           times_bought)(publishing_fee_escrow)(cd)(
                           seeder_price))

FC_REFLECT(steemit::chain::content_summary,
           (id)(author)(price)(synopsis)(status)(URI)(_hash)(AVG_rating)(size)(
                   expiration)(created)(times_bought))
FC_REFLECT(steemit::chain::PriceRegions, (map_price))
FC_REFLECT(steemit::chain::ContentObjectTypeValue, (type))

FC_REFLECT(steemit::chain::content_keys, (key)(parts))
