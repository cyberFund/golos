...
namespace steemit {
    namespace chain {
        ...
        void witness_schedule_policy::reset_virtual_schedule_time() {
        ...
        void witness_schedule_policy::update_median_witness_props() {
            const witness_schedule_object &wso = get_witness_schedule_object();
            ...
            for (int i = 0; i < wso.num_scheduled_witnesses; i++) {
                active.push_back(&get_witness(references, wso.current_shuffled_witnesses[i]));
            }
        ...
        void witness_schedule_policy::update_witness_schedule4() {
            const witness_schedule_object &wso = get_witness_schedule_object();
            ...
            references.modify(wso, [&](witness_schedule_object &_wso) {
                ...
                _wso.witness_pay_normalization_factor =
                        _wso.top19_weight * num_elected + _wso.miner_weight * num_miners +
        ...
        void witness_schedule_policy::update_witness_schedule() {
            if ((references.head_block_num() % STEEMIT_MAX_WITNESSES) == 0) //wso.next_shuffle_block_num )
            {
                ...
                references.modify(wso, [&](witness_schedule_object &_wso) {
                    ...
                    if (props.num_pow_witnesses == 0 ||
                        references.head_block_num() > STEEMIT_START_MINER_VOTING_BLOCK) {
                        _wso.current_virtual_time = new_virtual_time;
                    }
        ...
        const witness_schedule_object &witness_schedule_policy::get_witness_schedule_object() const {
            try {
