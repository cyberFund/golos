    ...
    namespace chain {
        ...
        void reward_policy::pay_liquidity_reward() {
            ...
            if ((references.head_block_num() % STEEMIT_LIQUIDITY_REWARD_BLOCKS) == 0) {
                ...
                const auto &ridx = references.get_index<liquidity_reward_balance_index>().indices().get<
                        by_volume_weight>();
                auto itr = ridx.begin();
                if (itr != ridx.end() && itr->volume_weight() > 0) {
                    ...
                    references.dynamic_extension_worker().get("account")->invoke("adjust_balance",
                                                                                 references.get(itr->owner), reward);
                    ...
                    references.push_virtual_operation(
                            liquidity_reward_operation(references.get(itr->owner).name, reward));
                }
            }
        }
