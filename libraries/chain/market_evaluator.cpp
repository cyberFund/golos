...
namespace steemit {
    namespace chain {
        ...
        void convert_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            ...
            FC_ASSERT(this->db.get_balance(owner, o.amount.symbol) >= o.amount,
                      "Account ${n} does not have sufficient balance for conversion. Balance: ${b}. Required: ${r}",
                      ("n", o.owner)("b", this->db.get_balance(owner, o.amount.symbol))("r", o.amount));
        ...
        void limit_order_create_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &op) {
            if (this->db.has_hardfork(STEEMIT_HARDFORK_0_17__115)) {
                try {
                ...
                try {
                    ...
                    this->db.modify(seller_stats, [&](account_statistics_object &bal) {
                        if (op.amount_to_sell.symbol == STEEM_SYMBOL_NAME) {
                            bal.total_core_in_orders += op.amount_to_sell.amount;
                        }
                    });

                    this->db.adjust_balance(this->db.get_account(op.owner), -op.amount_to_sell);

                    bool filled = this->db.apply_order(this->db.create<limit_order_object>([&](limit_order_object &obj) {
                ...
                FC_ASSERT(op.expiration > this->db.head_block_time(),
                          "Limit order has to expire after head block time.");

                const auto &owner = this->db.get_account(op.owner);

                FC_ASSERT(this->db.get_balance(owner, op.amount_to_sell.symbol) >= op.amount_to_sell,
        ...
        void call_order_update_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &op) {
            try {
                _paying_account = this->db.find_account(op.funding_account);
                _debt_asset = this->db.find_asset(op.delta_debt.symbol);

                FC_ASSERT(_debt_asset->is_market_issued(),
                          "Unable to cover ${sym} as it is not a collateralized asset.",
                          ("sym", _debt_asset->asset_name));

                _bitasset_data = this->db.find_asset_bitasset_data(_debt_asset->asset_name);
                ...
                if (op.delta_debt.amount < 0) {
                    FC_ASSERT(this->db.get_balance(*_paying_account, *_debt_asset) >= op.delta_debt,
                              "Cannot cover by ${c} when payer only has ${b}",
                              ("c", op.delta_debt.amount)("b", this->db.get_balance(*_paying_account, *_debt_asset).amount));
                }

                if (op.delta_collateral.amount > 0) {
                    FC_ASSERT(
                            this->db.get_balance(*_paying_account, this->db.get_asset(_bitasset_data->options.short_backing_asset)) >=
                            op.delta_collateral, "Cannot increase collateral by ${c} when payer only has ${b}",
                            ("c", op.delta_collateral.amount)("b", this->db.get_balance(*_paying_account, this->db.get_asset(
        ...
        void bid_collateral_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            const account_object &paying_account = this->db.get_account(o.bidder);

            try {
                const asset_object &debt_asset = this->db.get_asset(o.debt_covered.symbol);
                FC_ASSERT(debt_asset.is_market_issued(),
                          "Unable to cover ${sym} as it is not a collateralized asset.",
                          ("sym", debt_asset.asset_name));

                const asset_bitasset_data_object &bitasset_data = this->db.get_asset_bitasset_data(debt_asset.asset_name);
                ...
                if (o.additional_collateral.amount > 0) {
                    FC_ASSERT(this->db.get_balance(paying_account, this->db.get_asset_bitasset_data(
                            bitasset_data.options.short_backing_asset).asset_name) >= o.additional_collateral,
                              ...
                              ("c", o.additional_collateral.amount)("b", this->db.get_balance(paying_account, this->db.get_asset(
                                      o.additional_collateral.symbol)).amount));
                }

                const auto &index = this->db.get_index<collateral_bid_index>().indices().get<by_account>();
            ...
            try {
                ...
                this->db.adjust_balance(paying_account, -o.additional_collateral);
                _bid = &this->db.create<collateral_bid_object>([&](collateral_bid_object &bid) {
                    bid.bidder = o.bidder;
