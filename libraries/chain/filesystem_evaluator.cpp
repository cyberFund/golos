#include <steemit/chain/filesystem_evaluator.hpp>

#include <steemit/protocol/filesystem.hpp>
...
#include <steemit/chain/buying_object.hpp>
#include <steemit/chain/seeder_object.hpp>
#include <steemit/chain/content_object.hpp>
#include <steemit/chain/rating_object.hpp>
#include <steemit/chain/subscription_object.hpp>
#include <steemit/chain/seeding_statistics_object.hpp>
#include <steemit/chain/transaction_detail_object.hpp>

#include <steemit/encrypt/encryptionutils.hpp>
...
namespace steemit {
    namespace chain {
        ...
        void set_publishing_manager_evaluator::do_evaluate(const set_publishing_manager_operation &o) {
            try {
            ...
            try {
                for (auto to_id : o.to) {
                    ...
                    if (o.can_create_publishers == true) {
                        db().modify<account_object>(to_acc,
                                                    [](account_object &ao) {
                        ...
                        for (const account_id_type &publisher : to_acc.rights_to_publish.publishing_rights_forwarded) {
                            auto &publisher_acc = db().get<account_object>(
                                    publisher);
                            db().modify<account_object>(publisher_acc,
                                                        [&](account_object &ao) {
                                                            ao.rights_to_publish.publishing_rights_received.erase(
                                                                    publisher);
                                                        });
                        }
                        db().modify<account_object>(to_acc,
        ...
        void set_publishing_right_evaluator::do_apply(const set_publishing_right_operation &o) {
            try {
                ...
                for (const account_id_type &element : o.to) {
                    ...
                    if (o.is_publisher) {
                        db().modify<account_object>(from_acc,
                                                    [&](account_object &ao) {
                                                        ao.rights_to_publish.publishing_rights_forwarded.insert(
                                                                element);
                                                    });
                        db().modify<account_object>(to_acc,
                                                    [&](account_object &ao) {
                                                        ao.rights_to_publish.publishing_rights_received.insert(
                                                                o.from);
                                                    });
                    } else {
                        db().modify<account_object>(from_acc,
                                                    [&](account_object &ao) {
                                                        ao.rights_to_publish.publishing_rights_forwarded.erase(
                                                                element);
                                                    });
                        db().modify<account_object>(to_acc,
                                                    [&](account_object &ao) {
        ...
        void content_submit_evaluator::do_evaluate(const content_submit_operation &o) {
            try {
                const account_object &author_account = db().get<account_object>(
                        o.author);
                ...
                if (!o.co_authors.empty()) {
                    ...
                    if (it == o.co_authors.end()) {
                        FC_ASSERT(sum_of_splits < 10000,
                                  "Sum of splits exceeds allowed limit ( no remaining basis points for author's payout split ).");
                    }
                        // if author is included in co_authors map
                    else {
                        FC_ASSERT(sum_of_splits == 10000,
                                  "Sum of splits doesn't have required value ( 10000 basis points).");
                    }
                }
                ...
                fc::microseconds duration = (o.expiration -
                                             db().head_block_time());
                uint64_t days = duration.to_seconds() / 3600 / 24;
                FC_ASSERT(days != 0,
                          "time to expiration has to be at least one day");
                ...
                if (content_itr != content_idx.end()) // is resubmit?
                {
                    ...
                    FC_ASSERT(
                            content_itr->key_parts.size() == o.seeders.size());
                    for (const auto &element : content_itr->key_parts) {
                        FC_ASSERT(std::find(o.seeders.begin(), o.seeders.end(),
                                            element.first) != o.seeders.end());
                    }
                    if (content_itr->cd) {
        ...
        void content_submit_evaluator::do_apply(const content_submit_operation &o) {
            try {
                graphene::chain::ContentObjectPropertyManager synopsis_parser(
                        o.synopsis);
                ...
                if (is_resubmit) {
                    ...
                    graphene::chain::ContentObjectPropertyManager old_synopsis_parser(
                            content_itr->synopsis);
                    ...
                    const auto &transaction_detail_itr = transaction_detail_idx.find(
                            old_title);

                    db().modify<transaction_detail_object>(
                            *transaction_detail_itr,
                            [&](transaction_detail_object &tdo) {
                    ...
                    db().modify<content_object>(*content_itr,
                                                [&](content_object &co) {
                                                    ...
                                                    auto it_no_regions = prices.find(
                                                            RegionCodes::OO_none);
                                                    if (it_no_regions !=
                                                        prices.end()) {
                    ...
                    const auto &content = db().create<content_object>(
                            [&](content_object &co) {  //create new content object and store all values from the operation
                                ...
                                auto itr1 = o.seeders.begin();
                                auto itr2 = o.key_parts.begin();
                                while (itr1 != o.seeders.end() &&
                                       itr2 != o.key_parts.end()) {
                                    co.key_parts.emplace(
                    ...
                    db().adjust_balance(o.author,
                                        -o.publishing_fee);  //pay the escrow from author's account
                    auto &idx = db().get_index_type<seeder_index>().indices().get<by_seeder>();
                    ...
                    for (const auto &p : o.seeders) {
                        const auto &itr = idx.find(p);
                        db().modify<seeder_object>(*itr,
                                                   [&](seeder_object &so) {
                                                       so.free_space -= o.size;
                                                   });
                        db().modify<content_object>(content,
                                                    [&](content_object &co) {
                                                        co.seeder_price.emplace(
                                                                std::make_pair(
                                                                        p,
                                                                        itr->price.amount));
                                                    });
                    ...
                    auto &d = db();
                    db().create<transaction_detail_object>(
                            [&o, &title, &d](transaction_detail_object &obj) {
        ...
        void request_to_buy_evaluator::do_evaluate(const request_to_buy_operation &o) {
            try {
                ...
                FC_ASSERT(!content->is_blocked, "content has been canceled");
                {
                    ...
                    const auto &subscription = range.find(
                            boost::make_tuple(o.consumer, content->author));
                ...
                FC_ASSERT(price->asset_id == asset_id_type(0) ||
                          ao.is_monitored_asset());
        ...
        void request_to_buy_evaluator::do_apply(const request_to_buy_operation &o) {
            try {
                ...
                const auto &object = db().create<buying_object>(
                        [&](buying_object &bo) { //create new buying object
                            bo.consumer = o.consumer;
                            bo.URI = o.URI;
                            bo.expiration_time =
                                    db().head_block_time() + 24 * 3600;
                            bo.pubKey = o.pubKey;
                            bo.price = price;
                            bo.paid_price = price;
                ...
                auto &d = db();
                db().create<transaction_detail_object>(
                        [&o, &d, &price](transaction_detail_object &obj) {
        ...
        void deliver_keys_evaluator::do_evaluate(const deliver_keys_operation &o) {
            try {
                ...
                FC_ASSERT(decent::encrypt::verify_delivery_proof(proof, firstK,
                                                                 secondK,
                                                                 seeder_pubKey,
                                                                 buyer_pubKey));
            } FC_CAPTURE_AND_RETHROW((o))
        ...
        void deliver_keys_evaluator::do_apply(const deliver_keys_operation &o) {
            try {
                ...
                const auto &buying = db().get<buying_object>(o.buying);
                bool expired = (buying.expiration_time <
                                db().head_block_time());
                auto &idx = db().get_index_type<content_index>().indices().get<by_URI>();
                const auto &content = idx.find(buying.URI);
                bool delivered;
                ...
                if (delivered) {
                    ...
                    if (content->co_authors.empty()) {
                        ...
                        for (auto const &element : content->co_authors) {
                            price_for_co_author =
                                    (price.amount.value * element.second) /
                                    10000ll;
                            db().adjust_balance(element.first,
                                                static_cast<share_type>(price_for_co_author));
            ...
            try {
                ...
                db().modify<content_object>(*content, [&](content_object &co) {
                    ...
                    if (co.num_of_ratings == 0) {
                        ...
                        co.AVG_rating = (co.AVG_rating * co.num_of_ratings +
                                         o.rating * 1000) /
                                        (co.num_of_ratings + 1);
                        co.num_of_ratings++;
                    }
                ...
                auto &d = db();
                db().create<transaction_detail_object>(
                        [&o, &d](transaction_detail_object &obj) {
        ...
        void ready_to_publish_evaluator::do_apply(const ready_to_publish_operation &o) {
            try {
                ...
                if (sor == idx.end()) { //this is initial publish request
                    auto stats = db().create<seeding_statistics_object>(
                            [&](seeding_statistics_object &sso) {
                                sso.seeder = o.seeder;
                                sso.total_upload = 0;
                            }).id;
        ...
        void proof_of_custody_evaluator::do_evaluate(const proof_of_custody_operation &o) {
            try {
                ...
                if (o.proof.valid()) {
                    ...
                    fc::ripemd160 bid = db().get_block_id_for_num(
                            proof.reference_block);
                    for (int i = 0; i < 5; i++) {
                        FC_ASSERT(bid._hash[i] == proof.seed.data[i],
                                  "Block ID does not match; wrong chain?");
                    }
                    FC_ASSERT(
                            db().head_block_num() <= proof.reference_block + 6,
                            "Block reference is too old");
                }
                //
                FC_ASSERT(content->cd.valid() == o.proof.valid());
                FC_ASSERT(!(content->cd.valid()) ||
                          _custody_utils.verify_by_miner(*(content->cd),
                                                         *(o.proof)) == 0,
        ...
        void proof_of_custody_evaluator::do_apply(const proof_of_custody_operation &o) {
            try {
                ...
                if (last_proof == content->last_proof.end()) //initial PoR
                {
                    ...
                    fc::microseconds diff =
                            db().head_block_time() - last_proof->second;
                    if (diff > fc::days(1)) {
                        diff = fc::days(1);
                    }
                    uint64_t ratio = 10000 * diff.count() / fc::days(1).count();
                    uint64_t loss = (10000 - ratio) / 4;
                    uint64_t total_reward_ratio =
                            (ratio * (10000 - loss)) / 10000;
                    asset reward(0);
                    ...
                    if (db().head_block_num() > 404727) {
                        ...
                        reward.amount = itr->second * total_reward_ratio *
                                        content->size / 10000;
                    } else {
                        reward.amount =
                                seeder.price.amount * total_reward_ratio *
                                content->size / 10000;
                    }
                    //Fix due to block halt at #404726
                    if (db().head_block_num() > 404727 &&
                        reward.amount > content->publishing_fee_escrow.amount) {
                        reward.amount = std::max(int64_t(0),
                                                 content->publishing_fee_escrow.amount.value);
                    }
        ...
        void return_escrow_submission_evaluator::do_apply(const return_escrow_submission_operation &o) {
            void result;
            return result;
        }
        ...
        void report_stats_evaluator::do_apply(const report_stats_operation &o) {
            try {
                ...
                for (const auto &item : o.stats) {
                    ...
                    db().modify<seeding_statistics_object>(*so,
                                                           [&](seeding_statistics_object &sso) {
        ...
        void return_escrow_buying_evaluator::do_evaluate(const return_escrow_buying_operation &o) {
            void result;
            return result;
        }

        void return_escrow_buying_evaluator::do_apply(const return_escrow_buying_operation &o) {
            void result;
            return result;
        }

        void pay_seeder_evaluator::do_evaluate(const pay_seeder_operation &o) {
            void result;
            return result;
        }

        void pay_seeder_evaluator::do_apply(const pay_seeder_operation &o) {
            void result;
            return result;
        }

        void finish_buying_evaluator::do_evaluate(const finish_buying_operation &o) {
            void result;
            return result;
        }

        void finish_buying_evaluator::do_apply(const finish_buying_operation &o) {
            void result;
            return result;
        }
