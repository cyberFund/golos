...
namespace steemit {
    namespace chain {
        ...
        void_result proposal_create_evaluator::do_evaluate(const protocol::proposal_create_operation &o) {
            try {
                const database &d = db();
                const auto &global_parameters = d.get_global_properties().parameters;

                FC_ASSERT(o.expiration_time >
                          d.head_block_time(), "Proposal has already expired on creation.");
                FC_ASSERT(o.expiration_time <= d.head_block_time() +
                                               global_parameters.maximum_proposal_lifetime,
                        "Proposal expiration time is too far in the future.");
                FC_ASSERT(!o.review_period_seconds ||
                          fc::seconds(*o.review_period_seconds) <
                          (o.expiration_time - d.head_block_time()),
        ...
        object_id_type proposal_create_evaluator::do_apply(const protocol::proposal_create_operation &o) {
            try {
                database &d = db();
                ...
                const proposal_object &proposal = d.create<proposal_object>([&](proposal_object &proposal) {
                    ...
                    if (o.review_period_seconds) {
                        proposal.review_period_time =
                                o.expiration_time - *o.review_period_seconds;
                    }
        ...
        void_result proposal_update_evaluator::do_evaluate(const protocol::proposal_update_operation &o) {
            try {
                database &d = db();
        ...
        void_result proposal_update_evaluator::do_apply(const protocol::proposal_update_operation &o) {
            try {
                database &d = db();
        ...
        void_result proposal_delete_evaluator::do_evaluate(const protocol::proposal_delete_operation &o) {
            try {
                database &d = db();
                ...
                FC_ASSERT(required_approvals->find(o.fee_paying_account) !=
        ...
        void_result proposal_delete_evaluator::do_apply(const protocol::proposal_delete_operation &o) {
            try {
                db().remove(*_proposal);
