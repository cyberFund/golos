...
namespace steemit {
    namespace chain {
            ...
            try {
                ...
                if (dotpos != std::string::npos) {
                    ...
                    FC_ASSERT(asset_symbol_sub_itr->issuer == op.issuer,
                              ...
                              ("s", op.asset_name)("p", prefix)("i", this->db.template get_account(op.issuer).name));
                }

                if (op.bitasset_opts) {
                    const asset_object &backing = this->db.template get_asset(op.bitasset_opts->short_backing_asset);
                    if (backing.is_market_issued()) {
                        const asset_bitasset_data_object &backing_bitasset_data = this->db.template get_asset_bitasset_data(
                                backing.asset_name);
                        ...
                        FC_ASSERT(this->db.template get_account(op.issuer).name != STEEMIT_COMMITTEE_ACCOUNT ||
                ...
                if (op.is_prediction_market) {
                    ...
                    FC_ASSERT(op.precision == this->db.template get_asset(op.bitasset_opts->short_backing_asset).precision);
                }
        ...
        void asset_issue_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            try {
                const asset_object &a = this->db.template get_asset(o.asset_to_issue.symbol);
                FC_ASSERT(o.issuer == a.issuer);
                FC_ASSERT(!a.is_market_issued(), "Cannot manually issue a market-issued asset.");

                to_account = this->db.template find_account(o.issue_to_account);
                FC_ASSERT(this->db.template is_authorized_asset(*to_account, a));

                asset_dyn_data = this->db.template find_asset_dynamic_data(a.asset_name);
                FC_ASSERT((asset_dyn_data->current_supply + o.asset_to_issue.amount) <= a.options.max_supply);
            } FC_CAPTURE_AND_RETHROW((o))

            try {
                this->db.template adjust_balance(this->db.template get_account(o.issue_to_account), o.asset_to_issue);

                this->db.template modify(*asset_dyn_data, [&](asset_dynamic_data_object &data) {
        ...
        void asset_reserve_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            try {
                ...
                FC_ASSERT(this->db.template is_authorized_asset(*from_account, a));
            ...
            try {
                this->db.template adjust_balance(this->db.template get_account(o.payer), -o.amount_to_reserve);
        ...
        void asset_update_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            try {
                const asset_object &a = this->db.template get_asset(o.asset_to_update);
                auto a_copy = a;
                a_copy.options = o.new_options;
                a_copy.validate();

                if (o.new_issuer) {
                    FC_ASSERT(this->db.template find_account(*o.new_issuer));
                    if (a.is_market_issued() && *o.new_issuer == STEEMIT_COMMITTEE_ACCOUNT) {
                        const asset_object &backing = this->db.template get_asset(
                                this->db.template get_asset_bitasset_data(a.asset_name).options.short_backing_asset);
                        if (backing.is_market_issued()) {
                            const asset_object &backing_backing = this->db.template get_asset(
                                    this->db.template get_asset_bitasset_data(backing.asset_name).options.short_backing_asset);
                ...
                if ((this->db.template get_asset_dynamic_data(a.asset_name).current_supply != 0)) {
                ...
                if (o.new_options.flags & disable_force_settle && asset_to_update->can_force_settle()) {
                    ...
                    for (auto itr = idx.lower_bound(o.asset_to_update);
                         itr != idx.end() && itr->settlement_asset_symbol() == o.asset_to_update; itr = idx.lower_bound(
                            o.asset_to_update)) {
                        this->db.template cancel_order(*itr);
                    }
                }
        ...
        void asset_update_bitasset_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            try {
                ...
                const asset_bitasset_data_object &b = this->db.template get_asset_bitasset_data(a.asset_name);
                ...
                if (o.new_options.short_backing_asset != b.options.short_backing_asset) {
                    FC_ASSERT(this->db.template get_asset_dynamic_data(a.asset_name).current_supply == 0);
                    FC_ASSERT(this->db.template find_asset(o.new_options.short_backing_asset));
            ...
            try {
                ...
                this->db.template modify(*bitasset_to_update, [&](asset_bitasset_data_object &b) {
                    b.options = o.new_options;

                    if (should_update_feeds) {
                        b.update_median_feeds(this->db.template head_block_time());
                    }
                });
        ...
        void asset_update_feed_producers_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            ...
            try {
                this->db.template modify(*bitasset_to_update, [&](asset_bitasset_data_object &a) {
                    ...
                    a.update_median_feeds(this->db.template head_block_time());
                });
                this->db.template check_call_orders(this->db.template get_asset(o.asset_to_update));
        ...
        void asset_global_settle_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &op) {
            ...
            try {
                this->db.template globally_settle_asset(this->db.template get_asset(op.asset_to_settle), op.settle_price);
            } FC_CAPTURE_AND_RETHROW((op))
        }
        ...
        void asset_settle_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &op) {
            try {
                ...
                FC_ASSERT(asset_to_settle->is_market_issued());
                const auto &bitasset = this->db.template get_asset_bitasset_data(asset_to_settle->asset_name);
            ...
            try {
                this->db.template adjust_balance(this->db.template get_account(op.account), -op.amount);

                const auto &bitasset = this->db.template get_asset_bitasset_data(asset_to_settle->asset_name);
                if (bitasset.has_settlement()) {
                    const auto &mia_dyn = this->db.template get_asset_dynamic_data(asset_to_settle->asset_name);
                    ...
                    this->db.template modify(bitasset, [&](asset_bitasset_data_object &obj) {
                        obj.settlement_fund -= settled_amount.amount;
                    });

                    this->db.template adjust_balance(this->db.template get_account(op.account), settled_amount);

                    this->db.template modify(mia_dyn, [&](asset_dynamic_data_object &obj) {
            ...
            try {
                ...
                if (bitasset.has_settlement()) {
                    ...
                    this->db.template create<force_settlement_object>([&](force_settlement_object &s) {
                        ...
                        s.balance = op.amount;
                        s.settlement_date = this->db.template head_block_time() + this->db.template get_asset_bitasset_data(
        ...
        void asset_publish_feeds_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            try {
                const asset_object &base = this->db.template get_asset(o.asset_name);
                //Verify that this feed is for a market-issued asset and that asset is backed by the base
                FC_ASSERT(base.is_market_issued());

                const asset_bitasset_data_object &bitasset = this->db.template get_asset_bitasset_data(base.asset_name);
                if (bitasset.is_prediction_market) {
            ...
            try {
                const asset_object &base = this->db.template get_asset(o.asset_name);
                const asset_bitasset_data_object &bad = this->db.template get_asset_bitasset_data(base.asset_name);
                ...
                if (!(old_feed == bad.current_feed)) {
                    ...
                    this->db.template check_call_orders(base);
                }
        ...
        void asset_claim_fees_evaluator<Major, Hardfork, Release>::do_apply(const operation_type &o) {
            try {
                FC_ASSERT(this->db.template get_asset(o.amount_to_claim.symbol).issuer == o.issuer,
                          "Asset fees may only be claimed by the issuer");
            } FC_CAPTURE_AND_RETHROW((o))

            try {
                const asset_object &a = this->db.template get_asset(o.amount_to_claim.symbol);
                const asset_dynamic_data_object &addo = this->db.template get_asset_dynamic_data(a.asset_name);
                FC_ASSERT(o.amount_to_claim.amount <= addo.accumulated_fees,
                          "Attempt to claim more fees than have accumulated", ("addo", addo));

                this->db.template modify(addo, [&](asset_dynamic_data_object &_addo) {
                    _addo.accumulated_fees -= o.amount_to_claim.amount;
                });

                this->db.template adjust_balance(this->db.template get_account(o.issuer), o.amount_to_claim);
            } FC_CAPTURE_AND_RETHROW((o))
        }
    }
} // steemit::chain
