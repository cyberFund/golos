...
void steemit::chain::request_account_recovery_evaluator::do_apply(
        const protocol::request_account_recovery_operation &o) {
    ...
    if (account_to_recover.recovery_account.length()) {   // Make sure recovery matches expected recovery account
        ...
        FC_ASSERT(
                this->_db.get_index<witness_index>().indices().get<by_vote_name>().begin()->owner == o.recovery_account,
    ...
    if (request == recovery_request_idx.end()) // New Request
    {
        ...
        this->_db.create<account_recovery_request_object>([&](account_recovery_request_object &req) {
            ...
            req.expires = this->_db.head_block_time() + STEEMIT_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
        });
