...
void steemit::chain::vote_evaluator::do_apply(const protocol::vote_operation &o) {
    try {
        ...
        if (this->_db.has_hardfork(STEEMIT_HARDFORK_0_12__177) &&
            this->_db.calculate_discussion_payout_time(comment) == fc::time_point_sec::maximum()) {
#ifndef CLEAR_VOTES
        ...
        int64_t elapsed_seconds = (this->_db.head_block_time() - voter.last_vote_time).to_seconds();
        ...
        if (!this->_db.has_hardfork(STEEMIT_HARDFORK_0_14__259) && abs_rshares == 0) {
            abs_rshares = 1;
        }
        ...
        if (itr != comment_vote_idx.end() && itr->num_changes == -1) {
            if (this->_db.is_producing() || this->_db.has_hardfork(STEEMIT_HARDFORK_0_12__177)) {
        ...
        if (itr == comment_vote_idx.end()) {
            ...
            if (rshares > 0 && this->_db.has_hardfork(STEEMIT_HARDFORK_0_7)) {
                if (this->_db.has_hardfork(STEEMIT_HARDFORK_0_17__91)) {
                    FC_ASSERT(this->_db.head_block_time() < comment.cashout_time - STEEMIT_UPVOTE_LOCKOUT,
                              "Cannot increase reward of post within the last minute before payout.");
                } else {
                    FC_ASSERT(this->_db.head_block_time() <
                              this->_db.calculate_discussion_payout_time(comment) - STEEMIT_UPVOTE_LOCKOUT,
                              "Cannot increase reward of post within the last minute before payout.");
                }
            ...
            if (!this->_db.has_hardfork(STEEMIT_HARDFORK_0_17__91)) {
                ...
                if (this->_db.has_hardfork(STEEMIT_HARDFORK_0_12__177) &&
                    ...
                    new_cashout_time_sec =
                            this->_db.head_block_time().sec_since_epoch() + STEEMIT_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                } else {
                    new_cashout_time_sec =
                            this->_db.head_block_time().sec_since_epoch() + STEEMIT_CASHOUT_WINDOW_SECONDS_PRE_HF12;
                }

                avg_cashout_sec = (cur_cashout_time_sec * old_root_abs_rshares + new_cashout_time_sec * abs_rshares) /
            ...
            this->_db.modify(comment, [&](comment_object &c) {
                ...
                if (!this->_db.has_hardfork(STEEMIT_HARDFORK_0_6__114) && c.net_rshares == -c.abs_rshares) {
                ...
                if (!this->_db.has_hardfork(STEEMIT_HARDFORK_0_17__91)) {
                    if (this->_db.has_hardfork(STEEMIT_HARDFORK_0_12__177) &&
                        ...
                        c.cashout_time = fc::time_point_sec(
                                std::min(uint32_t(avg_cashout_sec.to_uint64()), c.max_cashout_time.sec_since_epoch()));
                    }

                    if (c.max_cashout_time == fc::time_point_sec::maximum()) {
                        c.max_cashout_time =
                                this->_db.head_block_time() + fc::seconds(STEEMIT_MAX_CASHOUT_WINDOW_SECONDS);
                    }
                }
            });
...
**/
            this->_db.create<comment_vote_object>([&](comment_vote_object &cv) {
                ...
                if (curation_reward_eligible && this->_db.has_hardfork(STEEMIT_HARDFORK_0_17__86)) {
                    curation_reward_eligible = this->_db.get_curation_rewards_percent(comment) > 0;
                }
                ...
                if (curation_reward_eligible) {
                    if (comment.created < fc::time_point_sec(STEEMIT_HARDFORK_0_6_REVERSE_AUCTION_TIME)) {
                        ...
                        if (this->_db.has_hardfork(STEEMIT_HARDFORK_0_1)) {
                            uint64_t old_weight = (
                                    (std::numeric_limits<uint64_t>::max() * fc::uint128_t(old_vote_rshares.value)) /
                                    (2 * two_s + old_vote_rshares.value)).to_uint64();
                            uint64_t new_weight = (
                                    (std::numeric_limits<uint64_t>::max() * fc::uint128_t(comment.vote_rshares.value)) /
                            ...
                            uint64_t new_weight = ((std::numeric_limits<uint64_t>::max() *
                                                   ...
                                                   (2 * two_s + (10000 * comment.vote_rshares.value))).to_uint64();
                            cv.weight = new_weight - old_weight;
                        }
                    }
                    ...
                    if (this->_db.head_block_time() > fc::time_point_sec(
            ...
            if (this->_db.is_producing() || this->_db.has_hardfork(STEEMIT_HARDFORK_0_6__112)) {
            ...
            if (itr->rshares < rshares && this->_db.has_hardfork(STEEMIT_HARDFORK_0_7)) {
                FC_ASSERT(this->_db.head_block_time() <
                          this->_db.calculate_discussion_payout_time(comment) - STEEMIT_UPVOTE_LOCKOUT,
