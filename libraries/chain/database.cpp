            ...
            while (current != cidx.end() && current->cashout_time <= head_block_time()) {
                ...
                if (has_hardfork(STEEMIT_HARDFORK_0_17__89)) {
                    ...
                    while (itr != com_by_root.end() && itr->root_comment == current->root_comment) {
                        ...
                        if (funds.size()) {
                            funds[fund_id].recent_claims += utilities::calculate_claims(current->net_rshares.value,
                                                                                        get_reward_fund(*current));
                        }
            ...
            if (!median_price.is_null() && has_hardfork(STEEMIT_HARDFORK_0_14__230)) {
                auto percent_sbd = uint16_t(((fc::uint128_t(
                        (dgp.current_sbd_supply * get_feed_history().current_median_history).amount.value) *
                                              STEEMIT_100_PERCENT) / dgp.virtual_supply.amount.value).to_uint64());
            ...
            if (log_head_num < dpo.last_irreversible_block_num) {
                while (log_head_num < dpo.last_irreversible_block_num) {
                    ...
                    if (blocks.size() == 1) {
                        ...
                        ilog("Encountered a block num collision due to a fork. Walking the current fork to determine the correct block. block_num:${n}",
                             ("n", log_head_num + 1)); // TODO: Delete when we know this code works as intended
                        auto next = _fork_db.head();
                        while (next.get() != nullptr && next->num > log_head_num + 1) {
                            next = next->prev.lock();
                        }

                        FC_ASSERT(next.get() != nullptr,
...
int
database::match(const limit_order_object &new_order, const limit_order_object &old_order, const price &match_price) {
    ...
    push_virtual_operation(fill_order_operation(new_order.seller, new_order.orderid, new_order_pays, old_order.seller,
            ...
            case SBD_SYMBOL:
                if (a.savings_sbd_seconds_last_update != head_block_time()) {
                    acnt.savings_sbd_seconds += fc::uint128_t(a.savings_sbd_balance.amount.value) *
                                                (head_block_time() - a.savings_sbd_seconds_last_update).to_seconds();
                    acnt.savings_sbd_seconds_last_update = head_block_time();

                    if (acnt.savings_sbd_seconds > 0 &&
                        (acnt.savings_sbd_seconds_last_update - acnt.savings_sbd_last_interest_payment).to_seconds() >
                        STEEMIT_SBD_INTEREST_COMPOUND_INTERVAL_SEC) {
...
void database::init_hardforks() {
    ...
    FC_ASSERT(hardforks.last_hardfork <= STEEMIT_NUM_HARDFORKS, "Chain knows of more hardforks than configuration",
              ("hardforks.last_hardfork", hardforks.last_hardfork)("STEEMIT_NUM_HARDFORKS", STEEMIT_NUM_HARDFORKS));
    ...
    switch (hardfork) {
        ...
        case STEEMIT_HARDFORK_0_16: {
            ...
            for (const std::string &acc : hardfork16::get_compromised_accounts()) {
                ...
                modify(get<account_authority_object, by_account>(account->name), [&](account_authority_object &auth) {
                    auth.active = authority(1, public_key_type("GLS8hLtc7rC59Ed7uNVVTXtF578pJKQwMfdTvuzYLwUi8GkNTh5F6"),
                                            1);
                    auth.posting = authority(1,
                                             public_key_type("GLS8hLtc7rC59Ed7uNVVTXtF578pJKQwMfdTvuzYLwUi8GkNTh5F6"),
                                             1);
                });
            }
            ...
            auto post_rf = create<reward_fund_object>([&](reward_fund_object &rfo) {
                ...
                rfo.last_update = head_block_time();
                rfo.content_constant = STEEMIT_CONTENT_CONSTANT_S_HF0;
        ...
        case STEEMIT_HARDFORK_0_17: {
            ...
            modify(get<reward_fund_object, by_name>(STEEMIT_POST_REWARD_FUND_NAME), [&](reward_fund_object &rfo) {
                ...
                rfo.reward_balance = asset(
                        (reward_steem.amount.value * rfo.percent_content_rewards) / STEEMIT_100_PERCENT, STEEM_SYMBOL);
    ...
    try {
        ...
        for (auto itr = account_idx.begin(); itr != account_idx.end(); ++itr) {
            ...
            total_vsf_votes += (itr->proxy == STEEMIT_PROXY_TO_SELF_ACCOUNT ? itr->witness_vote_weight() : (
                    STEEMIT_MAX_PROXY_RECURSION_DEPTH > 0 ? itr->proxied_vsf_votes[STEEMIT_MAX_PROXY_RECURSION_DEPTH -
                                                                                   1] : itr->vesting_shares.amount));
        }
        ...
        if (!get_feed_history().current_median_history.is_null()) {
            FC_ASSERT(gpo.current_sbd_supply * get_feed_history().current_median_history + gpo.current_supply ==
                      gpo.virtual_supply, "",
                      ("gpo.current_sbd_supply", gpo.current_sbd_supply)("get_feed_history().current_median_history",
