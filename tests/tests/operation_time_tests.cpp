...
BOOST_FIXTURE_TEST_SUITE(operation_time_tests, clean_database_fixture)
    ...
    BOOST_AUTO_TEST_CASE(comment_payout_equalize) {
        try {
            ...
            struct author_actor {
                author_actor(const std::string &n, fc::ecc::private_key pk,
                             fc::optional<latest_asset> mpay = fc::optional<latest_asset>()) : name(n), private_key(pk),
                        max_accepted_payout(mpay) {
                }

                std::string name;
                fc::ecc::private_key private_key;
                fc::optional<latest_asset> max_accepted_payout;
            };

            struct voter_actor {
                voter_actor(const std::string &n, fc::ecc::private_key pk, std::string fa) : name(n), private_key(pk),
                        favorite_author(fa) {
                }
            ...
            authors.emplace_back("bob", bob_private_key, latest_asset::from_string("0.000 TBD"));
            ...
            set_price_feed(
                    latest_price(latest_asset::from_string("0.001 TESTS"), latest_asset::from_string("1.000 TBD")));
            ...
            BOOST_CHECK(alice_account.sbd_balance == latest_asset::from_string("10720.000 TBD"));
            BOOST_CHECK(bob_account.sbd_balance == latest_asset::from_string("0.000 TBD"));
    ...
    BOOST_AUTO_TEST_CASE(comment_payout_dust) {
        try {
            ...
            set_price_feed(
                    latest_price(latest_asset::from_string("1.000 TESTS"), latest_asset::from_string("1.000 TBD")));
            ...
            BOOST_REQUIRE(db.get_dynamic_global_properties().total_reward_fund_steem ==
                          latest_asset::from_string("0.001 TESTS"));
    ...
    /*BOOST_AUTO_TEST_CASE( comment_payout )
    {
       try
       {
          ...
          price exchange_rate( latest_asset::from_string( "1.000 TESTS" ), latest_asset::from_string( "1.000 TBD" ) );
          ...
          auto bob_comment_payout = latest_asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_steem.amount.value ) / total_rshares2 ).to_uint64(), STEEM_SYMBOL_NAME );
          auto bob_comment_discussion_rewards = latest_asset( bob_comment_payout.amount / 4, STEEM_SYMBOL_NAME );
          bob_comment_payout -= bob_comment_discussion_rewards;
          auto bob_comment_sbd_reward = db.to_sbd( latest_asset( bob_comment_payout.amount / 2, STEEM_SYMBOL_NAME ) );
    ...
    BOOST_AUTO_TEST_CASE( comment_payout )
    {
       try
       {
          ...
          auto bob_comment_payout = latest_asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_steem.amount.value ) / total_rshares2 ).to_uint64(), STEEM_SYMBOL_NAME );
          auto bob_comment_vote_rewards = latest_asset( bob_comment_payout.amount / 2, STEEM_SYMBOL_NAME );
          ...
          auto alice_comment_payout = latest_asset( static_cast< uint64_t >( ( rf * rs2 ) / trs2 ), STEEM_SYMBOL_NAME );
          auto alice_comment_vote_rewards = latest_asset( alice_comment_payout.amount / 2, STEEM_SYMBOL_NAME );
    ...
    BOOST_AUTO_TEST_CASE( nested_comments )
    {
       try
       {
          ...
          auto alice_comment_total_payout = db.to_sbd( latest_asset( alice_pays_alice_sbd + alice_pays_alice_vest, STEEM_SYMBOL_NAME ) );
          alice_comment_total_payout += db.to_sbd( latest_asset( bob_pays_alice_sbd + bob_pays_alice_vest, STEEM_SYMBOL_NAME ) );
          alice_comment_total_payout += db.to_sbd( latest_asset( dave_pays_alice_sbd + dave_pays_alice_vest, STEEM_SYMBOL_NAME ) );
          auto bob_comment_total_payout = db.to_sbd( latest_asset( bob_pays_bob_sbd + bob_pays_bob_vest, STEEM_SYMBOL_NAME ) );
          bob_comment_total_payout += db.to_sbd( latest_asset( dave_pays_bob_sbd + dave_pays_bob_vest, STEEM_SYMBOL_NAME ) );
          auto sam_comment_total_payout = db.to_sbd( latest_asset( dave_pays_sam_sbd + dave_pays_sam_vest, STEEM_SYMBOL_NAME ) );
          auto dave_comment_total_payout = db.to_sbd( latest_asset( dave_pays_dave_sbd + dave_pays_dave_vest, STEEM_SYMBOL_NAME ) );
          ...
          auto alice_total_sbd = alice_starting_sbd + latest_asset( alice_pays_alice_sbd + bob_pays_alice_sbd + dave_pays_alice_sbd, STEEM_SYMBOL_NAME ) * exchange_rate;
          auto alice_total_vesting = alice_starting_vesting + latest_asset( alice_pays_alice_vest + bob_pays_alice_vest + dave_pays_alice_vest + alice_vote_alice_reward.amount + alice_vote_bob_reward.amount, STEEM_SYMBOL_NAME ) * gpo.get_vesting_share_price();
          ...
          auto bob_total_sbd = bob_starting_sbd + latest_asset( bob_pays_bob_sbd + dave_pays_bob_sbd, STEEM_SYMBOL_NAME ) * exchange_rate;
          auto bob_total_vesting = bob_starting_vesting + latest_asset( bob_pays_bob_vest + dave_pays_bob_vest + bob_vote_alice_reward.amount + bob_vote_bob_reward.amount + bob_vote_dave_reward.amount, STEEM_SYMBOL_NAME ) * gpo.get_vesting_share_price();
          ...
          auto sam_total_sbd = sam_starting_sbd + latest_asset( dave_pays_sam_sbd, STEEM_SYMBOL_NAME ) * exchange_rate;
          auto sam_total_vesting = bob_starting_vesting + latest_asset( dave_pays_sam_vest + sam_vote_bob_reward.amount, STEEM_SYMBOL_NAME ) * gpo.get_vesting_share_price();
          ...
          auto dave_total_sbd = dave_starting_sbd + latest_asset( dave_pays_dave_sbd, STEEM_SYMBOL_NAME ) * exchange_rate;
          auto dave_total_vesting = dave_starting_vesting + latest_asset( dave_pays_dave_vest, STEEM_SYMBOL_NAME ) * gpo.get_vesting_share_price();
    ...
    */
    BOOST_AUTO_TEST_CASE(vesting_withdrawals) {
        try {
            ...
            op.account = "alice";
            op.vesting_shares = latest_asset(new_alice.vesting_shares.amount / 2, VESTS_SYMBOL);
            tx.set_expiration(db.head_block_time() + STEEMIT_MAX_TIME_UNTIL_EXPIRATION);
            ...
            latest_asset vesting_shares = new_alice.vesting_shares;
            latest_asset to_withdraw = op.vesting_shares;
            latest_asset original_vesting = vesting_shares;
            latest_asset withdraw_rate = new_alice.vesting_withdraw_rate;
            ...
            auto fill_op = get_last_operations(1)[0].get<fill_vesting_withdraw_operation<0, 17, 0>>();
            auto gpo = db.get_dynamic_global_properties();
            ...
            BOOST_REQUIRE(
                    std::abs((fill_op.deposited - fill_op.withdrawn * gpo.get_vesting_share_price()).amount.value) <=
                    1);
            validate_database();
            ...
            for (int i = 1; i < STEEMIT_VESTING_WITHDRAW_INTERVALS - 1; i++) {
                ...
                gpo = db.get_dynamic_global_properties();
                fill_op = get_last_operations(1)[0].get<fill_vesting_withdraw_operation<0, 17, 0>>();
                ...
                BOOST_REQUIRE(fill_op.to_account == "alice");
                BOOST_REQUIRE(fill_op.withdrawn.amount.value == withdraw_rate.amount.value);
                BOOST_REQUIRE(std::abs(
                        (fill_op.deposited - fill_op.withdrawn * gpo.get_vesting_share_price()).amount.value) <= 1);
    ...
    BOOST_AUTO_TEST_CASE(vesting_withdraw_route) {
        try {
...

            {
                ...
                BOOST_REQUIRE(alice.balance == old_alice_balance + latest_asset(
                        (vesting_withdraw_rate.amount * STEEMIT_1_PERCENT * 20) / STEEMIT_100_PERCENT, VESTS_SYMBOL) *
                                                                   db.get_dynamic_global_properties().get_vesting_share_price());
                BOOST_REQUIRE(bob.vesting_shares == old_bob_vesting + latest_asset(
                        (vesting_withdraw_rate.amount * STEEMIT_1_PERCENT * 50) / STEEMIT_100_PERCENT, VESTS_SYMBOL));
                BOOST_REQUIRE(bob.balance == old_bob_balance);
                BOOST_REQUIRE(sam.vesting_shares == old_sam_vesting);
                BOOST_REQUIRE(sam.balance == old_sam_balance + latest_asset(
                        (vesting_withdraw_rate.amount * STEEMIT_1_PERCENT * 30) / STEEMIT_100_PERCENT, VESTS_SYMBOL) *
                                                               db.get_dynamic_global_properties().get_vesting_share_price());
    ...
    BOOST_AUTO_TEST_CASE(feed_publish_mean) {
        try {
            ...
            for (int i = 0; i < 7; i++) {
                transfer(STEEMIT_INIT_MINER_NAME, accounts[i], latest_asset(10000, STEEM_SYMBOL_NAME));
                witness_create(accounts[i], keys[i], "foo.bar", keys[i].get_public_key(), 1000);

                ops.push_back(feed_publish_operation<0, 17, 0>());
                ops[i].publisher = accounts[i];

                txs.push_back(signed_transaction());
            }

            ops[0].exchange_rate = latest_price(latest_asset(100000, STEEM_SYMBOL_NAME),
                                                latest_asset(1000, SBD_SYMBOL_NAME));
            ops[1].exchange_rate = latest_price(latest_asset(105000, STEEM_SYMBOL_NAME),
                                                latest_asset(1000, SBD_SYMBOL_NAME));
            ops[2].exchange_rate = latest_price(latest_asset(98000, STEEM_SYMBOL_NAME),
                                                latest_asset(1000, SBD_SYMBOL_NAME));
            ops[3].exchange_rate = latest_price(latest_asset(97000, STEEM_SYMBOL_NAME),
                                                latest_asset(1000, SBD_SYMBOL_NAME));
            ops[4].exchange_rate = latest_price(latest_asset(99000, STEEM_SYMBOL_NAME),
                                                latest_asset(1000, SBD_SYMBOL_NAME));
            ops[5].exchange_rate = latest_price(latest_asset(97500, STEEM_SYMBOL_NAME),
                                                latest_asset(1000, SBD_SYMBOL_NAME));
            ops[6].exchange_rate = latest_price(latest_asset(102000, STEEM_SYMBOL_NAME),
                                                latest_asset(1000, SBD_SYMBOL_NAME));
            ...
            BOOST_REQUIRE(feed_history.current_median_history ==
                          latest_price(latest_asset(99000, STEEM_SYMBOL_NAME), latest_asset(1000, SBD_SYMBOL_NAME)));
            ...
            for (int i = 0; i < 23; i++) {
                ...
                for (int j = 0; j < 7; j++) {
                    ...
                    ops[j].exchange_rate = latest_price(ops[j].exchange_rate.base,
                                                        latest_asset(ops[j].exchange_rate.quote.amount + 10,
                                                                     SBD_SYMBOL_NAME));
    ...
    BOOST_AUTO_TEST_CASE(convert_delay) {
        try {
            ...
            set_price_feed(latest_price(asset<0, 17, 0>::from_string("1.250 TESTS"),
                                        asset<0, 17, 0>::from_string("1.000 TBD")));
            ...
            tx.signatures.clear();
            op.owner = "alice";
            op.amount = latest_asset(2000, SBD_SYMBOL_NAME);
            op.request_id = 2;
            tx.operations.push_back(op);
            tx.set_expiration(db.head_block_time() + STEEMIT_MAX_TIME_UNTIL_EXPIRATION);
            tx.sign(alice_private_key, db.get_chain_id());
            ...
            generate_blocks(db.head_block_time() + STEEMIT_CONVERSION_DELAY - fc::seconds(STEEMIT_BLOCK_INTERVAL / 2),
                            true);
            ...
            auto vop = get_last_operations(1)[0].get<fill_convert_request_operation<0, 17, 0>>();
            ...
            BOOST_REQUIRE(vop.amount_in.amount.value == latest_asset::from_string("2.000 TBD").amount.value);
            BOOST_REQUIRE(vop.amount_out.amount.value == latest_asset::from_string("2.500 TESTS").amount.value);
            validate_database();
        } FC_LOG_AND_RETHROW();
    }
    ...
    BOOST_AUTO_TEST_CASE(steem_inflation) {
      ...
      auto new_rewards = std::max( STEEMIT_MIN_CONTENT_REWARD, latest_asset( ( STEEMIT_CONTENT_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL_NAME ) )
         + std::max( STEEMIT_MIN_CURATE_REWARD, latest_asset( ( STEEMIT_CURATE_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL_NAME ) );
      auto witness_pay = std::max( STEEMIT_MIN_PRODUCER_REWARD, latest_asset( ( STEEMIT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL_NAME ) );
      auto witness_pay_shares = latest_asset( 0, VESTS_SYMBOL );
      auto new_vesting_steem = latest_asset( 0, STEEM_SYMBOL_NAME );
      ...
      while( db.head_block_num() < STEEMIT_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( STEEMIT_MIN_CONTENT_REWARD, latest_asset( ( STEEMIT_CONTENT_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL_NAME ) )
            + std::max( STEEMIT_MIN_CURATE_REWARD, latest_asset( ( STEEMIT_CURATE_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL_NAME ) );
         witness_pay = std::max( STEEMIT_MIN_PRODUCER_REWARD, latest_asset( ( STEEMIT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL_NAME ) );
         new_vesting_steem = latest_asset( 0, STEEM_SYMBOL_NAME );
         ...
         if ( db.get_account( witness_name ).vesting_shares.amount.value == 0 )
         {
            ...
            witness_pay = latest_asset( 0, STEEM_SYMBOL_NAME );
         }
      ...
      while( db.head_block_num() < STEEMIT_START_MINER_VOTING_BLOCK - 1 )
      {
         ...
         witness_pay = std::max( STEEMIT_MIN_PRODUCER_REWARD, latest_asset( ( STEEMIT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL_NAME ) );
         auto witness_pay_shares = latest_asset( 0, VESTS_SYMBOL );
         new_vesting_steem = latest_asset( ( witness_pay + new_rewards ).amount * 9, STEEM_SYMBOL_NAME );
         new_vesting_shares = gpo.total_vesting_shares;
      ...
      for( int i = 0; i < STEEMIT_BLOCKS_PER_DAY; i++ )
      {
         ...
         witness_pay = std::max( STEEMIT_MIN_PRODUCER_REWARD, latest_asset( ( STEEMIT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( STEEMIT_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL_NAME ) );
         witness_pay_shares = witness_pay * gpo.get_vesting_share_price();
         new_vesting_steem = latest_asset( ( witness_pay + new_rewards ).amount * 9, STEEM_SYMBOL_NAME ) + witness_pay;
      ...
      BOOST_REQUIRE_EQUAL( db.get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( latest_asset( ( ( virtual_supply.amount.value / STEEMIT_BLOCKS_PER_YEAR ) * STEEMIT_1_PERCENT ) / STEEMIT_100_PERCENT, STEEM_SYMBOL_NAME ) * ( vesting_shares / vesting_steem ) ).amount.value );
      validate_database();
      */
        } FC_LOG_AND_RETHROW();
    }
    ...
    BOOST_AUTO_TEST_CASE(sbd_interest) {
        try {
            ...
            set_price_feed(latest_price(asset<0, 17, 0>::from_string("1.000 TESTS"),
                                        asset<0, 17, 0>::from_string("1.000 TBD")));
            ...
            transfer.to = "bob";
            transfer.from = "alice";
            transfer.amount = latest_asset::from_string("1.000 TBD");
            tx.operations.clear();
            tx.signatures.clear();
            tx.set_expiration(db.head_block_time() + STEEMIT_MAX_TIME_UNTIL_EXPIRATION);
            ...
            auto gpo = db.get_dynamic_global_properties();
            auto interest_op = get_last_operations(1)[0].get<interest_operation<0, 17, 0>>();

            BOOST_REQUIRE(gpo.sbd_interest_rate > 0);
            BOOST_REQUIRE(db.get_account("alice").sbd_balance.amount.value ==
                          alice_sbd.amount.value - latest_asset::from_string("1.000 TBD").amount.value +
                          ((((uint128_t(alice_sbd.amount.value) * (db.head_block_time() - start_time).to_seconds()) /
                             STEEMIT_SECONDS_PER_YEAR) * gpo.sbd_interest_rate) / STEEMIT_100_PERCENT).to_uint64());
            BOOST_REQUIRE(interest_op.owner == "alice");
            BOOST_REQUIRE(interest_op.interest.amount.value == db.get_account("alice").sbd_balance.amount.value -
                                                               (alice_sbd.amount.value -
                                                                latest_asset::from_string("1.000 TBD").amount.value));
            validate_database();
            ...
            generate_blocks(db.head_block_time() + fc::seconds((STEEMIT_SBD_INTEREST_COMPOUND_INTERVAL_SEC * 7) / 3),
                            true);
            ...
            BOOST_REQUIRE(db.get_account("alice").sbd_balance.amount.value ==
                          alice_sbd.amount.value - latest_asset::from_string("1.000 TBD").amount.value +
                          ((((uint128_t(alice_sbd.amount.value) * (db.head_block_time() - start_time).to_seconds() +
                              alice_coindays) / STEEMIT_SECONDS_PER_YEAR) * gpo.sbd_interest_rate) /
    ...
    BOOST_AUTO_TEST_CASE(liquidity_rewards) {
        ...
        try {
            ...
            auto exchange_rate = latest_price(latest_asset::from_string("1.250 TESTS"),
            ...
            op.owner = "alice";
            op.amount_to_sell = latest_asset(alice_sbd.amount.value / 20, SBD_SYMBOL_NAME);
            ...
            alice_steem_volume += (latest_asset(alice_sbd.amount / 20, SBD_SYMBOL_NAME) * exchange_rate).amount.value;
            alice_reward_last_update = db.head_block_time();
            bob_steem_volume -= (latest_asset(alice_sbd.amount / 20, SBD_SYMBOL_NAME) * exchange_rate).amount.value;
      ...
      BOOST_CHECK( reward->last_update == bob_reward_last_update );*/
            ...
            op.owner = "sam";
            op.amount_to_sell = latest_asset((alice_sbd.amount.value / 20), STEEM_SYMBOL_NAME);
            op.min_to_receive = latest_asset((alice_sbd.amount.value / 20), SBD_SYMBOL_NAME);
            op.order_id = 3;
            ...
            op.owner = "bob";
            op.order_id = 4;
            op.amount_to_sell = latest_asset((alice_sbd.amount.value / 10) * 3 - alice_sbd.amount.value / 20,
                                             STEEM_SYMBOL_NAME);
            op.min_to_receive = latest_asset((alice_sbd.amount.value / 10) * 3 - alice_sbd.amount.value / 20,
                                             SBD_SYMBOL_NAME);
            ...
            op.owner = "alice";
            op.order_id = 5;
            op.amount_to_sell = latest_asset((alice_sbd.amount.value / 10) * 3, SBD_SYMBOL_NAME);
            op.min_to_receive = latest_asset((alice_sbd.amount.value / 10) * 3, STEEM_SYMBOL_NAME);
      ...
      BOOST_CHECK( reward->last_update == sam_reward_last_update );*/
            ...
            op.order_id = 6;
            op.amount_to_sell = latest_asset(alice_sbd.amount.value / 20 * 2, SBD_SYMBOL_NAME);
            op.min_to_receive = latest_asset(alice_sbd.amount.value / 20 * 2, STEEM_SYMBOL_NAME);
      ...
      BOOST_CHECK( reward->last_update == sam_reward_last_update );*/
            ...
            transfer.to = "dave";
            transfer.from = "alice";
            transfer.amount = latest_asset(alice_sbd.amount / 2, SBD_SYMBOL_NAME);
            ...
            op.owner = "alice";
            op.amount_to_sell = latest_asset(8 * (alice_sbd.amount.value / 20), STEEM_SYMBOL_NAME);
            op.min_to_receive = latest_asset(op.amount_to_sell.amount, SBD_SYMBOL_NAME);
            op.order_id = 9;
            tx.operations.clear();
            tx.signatures.clear();
            tx.operations.push_back(op);
            ...
            op.owner = "dave";
            op.amount_to_sell = latest_asset(7 * (alice_sbd.amount.value / 20), SBD_SYMBOL_NAME);;
            op.min_to_receive = latest_asset(op.amount_to_sell.amount, STEEM_SYMBOL_NAME);
            op.order_id = 10;
            tx.operations.clear();
            tx.signatures.clear();
            tx.operations.push_back(op);
      ...
      BOOST_CHECK( reward->last_update == dave_reward_last_update );*/
            ...
            transfer.to = "bob";
            transfer.from = "alice";
            transfer.amount = latest_asset(alice_sbd.amount / 5, SBD_SYMBOL_NAME);
            tx.operations.clear();
            tx.signatures.clear();
            tx.operations.push_back(transfer);
            tx.set_expiration(db.head_block_time() + STEEMIT_MAX_TIME_UNTIL_EXPIRATION);
            ...
            op.owner = "bob";
            op.order_id = 12;
            op.amount_to_sell = latest_asset(3 * (alice_sbd.amount / 40), SBD_SYMBOL_NAME);
            op.min_to_receive = latest_asset(op.amount_to_sell.amount, STEEM_SYMBOL_NAME);
            tx.operations.clear();
            tx.signatures.clear();
            tx.operations.push_back(op);
      ...
      BOOST_CHECK( reward->last_update == dave_reward_last_update );*/
            ...
            STEEMIT_REQUIRE_THROW(ops[0].get<typename BOOST_IDENTITY_TYPE((liquidity_reward_operation<0, 17, 0>))>(),
                                  fc::exception);
            //BOOST_REQUIRE( ops[0].get< liquidity_reward_operation<0, 17, 0>>().payout.amount.value == STEEMIT_MIN_LIQUIDITY_REWARD.amount.value );
            ...
            generate_blocks(
                    sam_reward_last_update + STEEMIT_LIQUIDITY_TIMEOUT_SEC - fc::seconds(STEEMIT_BLOCK_INTERVAL / 2) -
      ...
      BOOST_CHECK( reward->last_update == sam_reward_last_update );*/
            ...
            op.min_to_receive.symbol = STEEM_SYMBOL_NAME;
            tx.operations.clear();
            tx.signatures.clear();
            tx.operations.push_back(op);
            tx.set_expiration(db.head_block_time() + STEEMIT_MAX_TIME_UNTIL_EXPIRATION);
            ...
            sam_sbd_volume = latest_asset::from_string("1.000 TBD").amount.value;
            sam_steem_volume = 0;
            sam_reward_last_update = db.head_block_time();
    ...
    BOOST_AUTO_TEST_CASE(post_rate_limit) {
        try {
            ...
            alice_post_bandwidth = STEEMIT_100_PERCENT + (alice_post_bandwidth * (STEEMIT_POST_AVERAGE_WINDOW -
            ...
            alice_post_bandwidth = STEEMIT_100_PERCENT + (alice_post_bandwidth * (STEEMIT_POST_AVERAGE_WINDOW -
                                                                                  ...
                                                                                  STEEMIT_BLOCK_INTERVAL) /
                                                          STEEMIT_POST_AVERAGE_WINDOW);
            auto reward_weight = (STEEMIT_POST_WEIGHT_CONSTANT_PRE_HF17 * STEEMIT_100_PERCENT) /
                                                                                                                            ...
                                                                                                                            BOOST_AUTO_TEST_CASE( sbd_stability )
{
   try
   {
      ...
      auto db_plugin = app.register_plugin< golos::plugin::debug_node::debug_node_plugin >();
      ...
      auto exchange_rate = latest_price( latest_asset::from_string( "1.000 TBD" ), latest_asset::from_string( "10.000 TESTS" ) );
      set_price_feed( exchange_rate );
      ...
      latest_asset sbd_balance = latest_asset( ( gpo.virtual_supply.amount * ( STEEMIT_SBD_STOP_PERCENT + 30 ) ) / STEEMIT_100_PERCENT, STEEM_SYMBOL_NAME ) * exchange_rate;
      ...
      auto alice_sbd = db.get_account( "alice" ).sbd_balance + latest_asset( sbd_reward, STEEM_SYMBOL_NAME ) * exchange_rate;
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( "sam" ), [&]( account_object& a )
         {
            a.sbd_balance = latest_asset( ( 194 * sbd_balance.amount ) / 500, SBD_SYMBOL_NAME );
         });
      }, database::skip_witness_signature );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.current_sbd_supply = alice_sbd + latest_asset( ( 194 * sbd_balance.amount ) / 500, SBD_SYMBOL_NAME );
         });
      }, database::skip_witness_signature );
   ...
   FC_LOG_AND_RETHROW()
    ...
    BOOST_AUTO_TEST_CASE(sbd_price_feed_limit) {
        try {
            ...
            price<0, 17, 0> exchange_rate(latest_asset::from_string("1.000 TBD"),
            ...
            auto new_exchange_rate = latest_price(gpo.current_sbd_supply,
                                                  latest_asset((STEEMIT_100_PERCENT) * gpo.current_supply.amount));
            set_price_feed(new_exchange_rate);
            set_price_feed(new_exchange_rate);

            BOOST_REQUIRE(db.get_feed_history().current_median_history > new_exchange_rate &&
    ...
    BOOST_AUTO_TEST_CASE(clear_null_account) {
        try {
            ...
            BOOST_REQUIRE(db.get_account(STEEMIT_NULL_ACCOUNT).balance == latest_asset::from_string("1.000 TESTS"));
            BOOST_REQUIRE(db.get_account(STEEMIT_NULL_ACCOUNT).sbd_balance == latest_asset::from_string("2.000 TBD"));
            BOOST_REQUIRE(
                    db.get_account(STEEMIT_NULL_ACCOUNT).vesting_shares > latest_asset::from_string("0.000000 VESTS"));
            BOOST_REQUIRE(
                    db.get_account(STEEMIT_NULL_ACCOUNT).savings_balance == latest_asset::from_string("4.000 TESTS"));
            BOOST_REQUIRE(
                    db.get_account(STEEMIT_NULL_ACCOUNT).savings_sbd_balance == latest_asset::from_string("5.000 TBD"));
            BOOST_REQUIRE(db.get_account("alice").balance == latest_asset::from_string("2.000 TESTS"));
            BOOST_REQUIRE(db.get_account("alice").sbd_balance == latest_asset::from_string("3.000 TBD"));

            BOOST_TEST_MESSAGE("--- Generating block to clear balances");
            generate_block();
            validate_database();

            BOOST_REQUIRE(db.get_account(STEEMIT_NULL_ACCOUNT).balance == latest_asset::from_string("0.000 TESTS"));
            BOOST_REQUIRE(db.get_account(STEEMIT_NULL_ACCOUNT).sbd_balance == latest_asset::from_string("0.000 TBD"));
            BOOST_REQUIRE(
                    db.get_account(STEEMIT_NULL_ACCOUNT).vesting_shares == latest_asset::from_string("0.000000 VESTS"));
            BOOST_REQUIRE(
                    db.get_account(STEEMIT_NULL_ACCOUNT).savings_balance == latest_asset::from_string("0.000 TESTS"));
            BOOST_REQUIRE(
                    db.get_account(STEEMIT_NULL_ACCOUNT).savings_sbd_balance == latest_asset::from_string("0.000 TBD"));
            BOOST_REQUIRE(db.get_account("alice").balance == latest_asset::from_string("2.000 TESTS"));
            BOOST_REQUIRE(db.get_account("alice").sbd_balance == latest_asset::from_string("3.000 TBD"));
        } FC_LOG_AND_RETHROW()
    }
    ...
    BOOST_AUTO_TEST_CASE(reward_funds) {
        try {
...

            {
                ...
                BOOST_REQUIRE(db.get_account("alice").sbd_balance.amount > 0);
                BOOST_REQUIRE(db.get_account("bob").sbd_balance.amount == 0);
                validate_database();
            }
